...
| `W` / `↑` | 视觉网格跨行快速向上跳转（-5 题） |
| `S` / `↓` | 视觉网格跨行快速向下跳转（+5 题） |
| `Q` | 切换上一章节（复习会话中为切换上一复习题） |
| `E` | 切换下一章节（复习会话中为切换下一复习题） |
| `F` | 切换父题模式 / 子题逐题模式（小题选择模式） |
| `滚轮左右推` | 触控板/鼠标横向手势快速切题 |

### 2. 掌握度评级（5 级标准色板）
| 按键组合 | 掌握度评级 | 颜色标识 |
| :---: | :---: | :---: |
| `Z` | **熟练** (lv5 完美) | 🟩 绿 (`#389E0D`) |
| `Z + X` | **较熟练** (lv4 正确但犹豫) | 🟢 黄绿 (`#7CB305`) |
| `X` | **模糊** (lv3 思路对但卡壳) | 🟨 黄 (`#FBC02D`) |
| `X + C` | **困难** (lv2 大部分忘记) | 🟧 橙 (`#F57C00`) |
| `C` | **不会** (lv1 全忘) | 🟥 红 (`#D32F2F`) |
| `Ctrl + Z` | 撤销最近一次掌握度标记 | — |
...
| `L` | 打开 / 关闭同类题与考点关联管理弹窗 |
| `M` | 打开 / 关闭 SM-2 间隔重复复习面板 |
| `V` | 打开 / 关闭全局进度看板 |
| `B` | 打开 / 关闭错题本面板 |
| `I` | 收起 / 展开左侧章节目录栏 |
| `P` | 收起 / 展开常用 LaTeX 数学符号代码盘 |
| `G` | 打开科目切换选择面板（数学 / 822 / 英语） |
| `Y` | 切换系统主题（明亮 / 暗夜） |
| `U` | 切换试卷图片暗化滤镜（关 / 开） |
| `H` | 打开 / 关闭快捷键帮助面板 |
| `Esc` | 关闭当前弹出的模态框、大图查看器或退出复习会话 |
...
| `滚轮` | 以光标为中心缩放图片 |
| `右键 + 滚轮` | 快速调节标注画笔粗细 |
| `Tab` / `左右推` | 快速切换画笔工具（直线 / 矩形 / 箭头 / 画笔） |
| `Shift + 拖动` | 绘制直线时锁定水平或竖直方向 |
| `Space` | 显示 / 隐藏标注工具栏 |
| `+` / `-` / `0` | 灯箱图片放大 / 缩小 / 还原 1:1 比例 |
| `Esc` | 退出标注模式 / 关闭灯箱 |
...
| :--- | :--- |
| `A` / `D` 或 `←` / `→` | 跨书查题网格上一题 / 下一题 |
| `W` / `S` 或 `↑` / `↓` | 跨书查题 5 列网格上移一行 / 下移一行 |
| `Space` | 切换查题区域当前题目解析显示状态 |
| `Enter` | 关联 / 移出当前题目至考点 |
| `L` 或 `Esc` | 关闭同类题管理弹窗 |

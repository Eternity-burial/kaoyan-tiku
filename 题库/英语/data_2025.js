/**
 * 考研英语 · 历年真题精读与题型分析数据库
 * 年份：2025 年真题 (按照 2010 年金牌标准纯手工与精品讲义全量重构)
 */

window.ENGLISH_DATA = window.ENGLISH_DATA || {};
window.ENGLISH_DATA['2025'] = {
  "year": 2025,
  "subject": "英语",
  "title": "2025年全国硕士研究生招生考试英语(一)真题阅读精读与题型分析",
  "texts": [
    {
      "id": "text1",
      "number": 1,
      "title": "The \"Rehearsal Room\" Approach to Shakespeare: Boosting Literacy and Emotional Literacy in Pupils",
      "chineseTitle": "莎士比亚戏剧“排练室法”与儿童读写与情感共情力发展",
      "topic": "教育心理学与戏剧教学 / 皇家莎士比亚剧团“排练室法”、角色扮演同理心与莎剧两万词汇对语言情感的全面赋能",
      "overview": "本文探讨了由皇家莎士比亚剧团（RSC）委托开展的一项关于莎士比亚戏剧沉浸式教学法的开创性随机对照实验（RCT）。研究表明，采用戏剧“排练室法”亲自演绎角色，不仅大幅拓宽了九至十岁小学生的词汇库并提升了写作复杂度，更显著增强了他们的情感共情素养。实验组学生在代入《暴风雨》斐迪南王子角色时，能够设身处地感同身受表达角色情感并展现出更宏大细腻的景物描摹力，而对照组学生则受困于“荒岛陈词滥调”暴露出描写功力的薄弱。专家指出，戏剧本质在于换位思考共情，而莎翁戏剧浩瀚的两万词汇结合全身体验，使得该教学法在莎剧中能够发挥出最卓越非凡的最佳成效。",
      "paragraphs": [
        {
          "pIndex": 1,
          "mainIdea": "开创性研究表明亲自演绎莎翁戏剧确实能显著促进儿童读写与情感共情力发展",
          "logicRole": "提出戏剧演绎莎士比亚对儿童语言与情感发展的开创性实证发现 (Shakespeare Benefits Children's Literacy)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P1-S1",
              "text": "The grammar school boy from Stratford-upon-Avon has landed a scholarly punch after ground-breaking research showed that Shakespeare does benefit children's literacy and emotional development. But only if you act him out.",
              "translation": "这位来自埃文河畔斯特拉特福德的昔日文法学校男孩（莎士比亚），在学术界结结实实地打出了一记漂亮的重拳——一项开创性的重磅研究表明：莎士比亚戏剧确实对儿童的读写能力与情感心智发育大有裨益（does benefit children's literacy and emotional development）。但这有一个绝对的前提：孩子们必须亲自将其搬上舞台演绎出来（play the roles in Shakespeare / act him out）！",
              "vocab": [
                {
                  "word": "scholarly",
                  "ipa": "/ˈskɒləli/",
                  "meaning": "学术的、学术研究性质的（常用释义：有学问的；学者式的）",
                  "level": "red"
                },
                {
                  "word": "punch",
                  "ipa": "/pʌntʃ/",
                  "meaning": "习语中指“关键出手/行动时机”（常用释义：一拳；冲击力；打孔器）",
                  "level": "green"
                },
                {
                  "word": "benefit",
                  "ipa": "ˈbenɪfɪt",
                  "meaning": "n. 好处，益",
                  "level": "green"
                },
                {
                  "word": "literacy",
                  "ipa": "ˈlɪtərəsi",
                  "meaning": "n.识字，读写能力",
                  "level": "green"
                },
                {
                  "word": "emotional",
                  "ipa": "ɪˈmoʊʃən(ə)l",
                  "meaning": "adj.情感的；情绪的；激动人心的；情绪激动的",
                  "level": "green"
                },
                {
                  "word": "act",
                  "ipa": "ækt",
                  "meaning": "v.表演;举动;起作用 n.行为,法令;一幕",
                  "level": "red"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 2,
          "mainIdea": "“排练室”教学法拓宽了儿童词汇量并提升其写作复杂度与情感共情素养",
          "logicRole": "皇家莎士比亚剧团介绍“排练室法”对语言和自我认知改变的显著效果 (Rehearsal Room Approach)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P2-S1",
              "text": "A study found that a \"rehearsal room\" approach to teaching Shakespeare broadened children's vocabulary and the complexity of their writing as well as their emotional literacy.",
              "translation": "一项实证研究发现，采用戏剧“排练室”（rehearsal room）沉浸式方法来教授莎士比亚戏剧，不仅大幅拓宽了儿童的词汇量，显著提升了其书面写作的复杂程度，更极大增强了他们的情感共情素养（emotional literacy）。",
              "vocab": [
                {
                  "word": "rehearsal",
                  "ipa": "rɪˈhɜːs(ə)l",
                  "meaning": "n.排练，排演；预演，演习；（~of）复述，重复",
                  "level": "green"
                },
                {
                  "word": "room",
                  "ipa": "ˈweɪtɪŋ- rʊm",
                  "meaning": "n.候诊室,",
                  "level": "green"
                },
                {
                  "word": "approach",
                  "ipa": "/əˈprəʊtʃ/",
                  "meaning": "研究方法；路径（常用释义：方法；接近；着手处理）",
                  "level": "red"
                },
                {
                  "word": "vocabulary",
                  "ipa": "",
                  "meaning": "n.词汇，词汇表",
                  "level": "red"
                },
                {
                  "word": "complexity",
                  "ipa": "/kəmˈpleksəti/",
                  "meaning": "复杂性（常用释义：复杂程度）",
                  "level": "green"
                },
                {
                  "word": "emotional",
                  "ipa": "ɪˈmoʊʃən(ə)l",
                  "meaning": "adj.情感的；情绪的；激动人心的；情绪激动的",
                  "level": "green"
                },
                {
                  "word": "literacy",
                  "ipa": "ˈlɪtərəsi",
                  "meaning": "n.识字，读写能力",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            },
            {
              "sIndex": 2,
              "id": "P2-S2",
              "text": "\"The research shows that the way actors work makes a big difference to the way children use language and also how they think about themselves,\" Jacqui O'Hanlon of the Royal Shakespeare Company (RSC), which commissioned the study, said.",
              "translation": "“这项研究雄辩地证明，专业演员的工作演绎方式，对孩子们如何运用语言以及他们如何审视看待自我，带来了极其深远重大的积极改变，”委托开展该项研究的皇家莎士比亚剧团（RSC）教育总监 Jacqui O'Hanlon 表示。",
              "vocab": [
                {
                  "word": "work",
                  "ipa": "wɜ:rk",
                  "meaning": "n.工作(量);作品;[pl.]工厂 v.工作;运转",
                  "level": "red"
                },
                {
                  "word": "use",
                  "ipa": "",
                  "meaning": "n./v. 使用，用途",
                  "level": "green"
                },
                {
                  "word": "language",
                  "ipa": "ˈlæŋɡwɪdʒ",
                  "meaning": "n. 语言",
                  "level": "green"
                },
                {
                  "word": "about",
                  "ipa": "əˈbaʊt",
                  "meaning": "prep.从事于,忙于",
                  "level": "red"
                },
                {
                  "word": "Royal",
                  "ipa": "ˈrɔɪəl",
                  "meaning": "adj.王室的,皇家的;第一流的,高贵的",
                  "level": "red"
                },
                {
                  "word": "Company",
                  "ipa": "ˈkəmpəˌni",
                  "meaning": "n. 公司 n. 剧团，表演",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 3,
          "mainIdea": "对照实验将数百名小学生分为实验组与对照组以探究教学法改变对学习成效的影响",
          "logicRole": "介绍45所公立小学分组对照实验设计 (RCT Study Design & Target vs Control)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P3-S1",
              "text": "The randomised control trial involved hundreds of year 5 pupils—aged nine and ten—at 45 state primary schools that had not been \"previously exposed to RSC pedagogy\".",
              "translation": "这项随机对照试验（RCT）涵盖了45所此前从未接触过皇家莎士比亚剧团教学法的公立小学里数百名九至十岁的五年级学生。",
              "vocab": [
                {
                  "word": "randomised",
                  "ipa": "ˈrændəmaɪzd",
                  "meaning": "adj.随机化的，随机的 v.使随机化（randomise 的过去式",
                  "level": "green"
                },
                {
                  "word": "trial",
                  "ipa": "ˈtraɪəl",
                  "meaning": "n.审判；试验；磨练；烦人的事/人；预赛 v.测试，试验",
                  "level": "green"
                },
                {
                  "word": "involved",
                  "ipa": "/ɪnˈvɒlvd/",
                  "meaning": "涉及，包含（常用释义：涉及；参与；复杂的）",
                  "level": "red"
                },
                {
                  "word": "state",
                  "ipa": "/steɪt/",
                  "meaning": "正式陈述；说明（常用释义：状态；州；国家；陈述）",
                  "level": "red"
                },
                {
                  "word": "primary",
                  "ipa": "/ˈpraɪməri/",
                  "meaning": "主要的；基本的（常用释义：首要的；初级的；初选的）",
                  "level": "red"
                },
                {
                  "word": "previously",
                  "ipa": "ˈpriːviəsli",
                  "meaning": "adv.以前地，先前地",
                  "level": "green"
                },
                {
                  "word": "pedagogy",
                  "ipa": "ˈpedəɡɒdʒi",
                  "meaning": "n.教育学，教学法",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P3-S2",
              "text": "They were split into target and control groups and asked to write, for example, a message in a bottle as Ferdinand after the shipwreck in The Tempest. The target group was given a 30-minute drama-based activity to accompany the passage.",
              "translation": "这些学生被随机分成实验目标组与对照组（to find whether the change in instruction enhances learning outcomes）；两组学生被要求以《暴风雨》中遭遇海难后的斐迪南王子的第一人称角色，写下一封漂流瓶信件。其中，实验目标组在阅读文本的同时，被安排参与了整整30分钟的沉浸式戏剧表演角色扮演活动。",
              "vocab": [
                {
                  "word": "split",
                  "ipa": "splɪt",
                  "meaning": "v.（使）分裂；（使）分开；（使）撕裂；分担，分享；划破，割",
                  "level": "green"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "message",
                  "ipa": "ˈmesɪdʒ",
                  "meaning": "n. 消息，音信",
                  "level": "green"
                },
                {
                  "word": "shipwreck",
                  "ipa": "ˈʃɪprek",
                  "meaning": "n.海难；遇难船 v.使失事；使毁灭；使失败",
                  "level": "green"
                },
                {
                  "word": "Tempest",
                  "ipa": "ˈtempɪst",
                  "meaning": "n.暴风雨；风波；暴动",
                  "level": "green"
                },
                {
                  "word": "drama-based",
                  "ipa": "ˈdrɑːməbeɪst",
                  "meaning": "adj.基于某个故事或事件改编的",
                  "level": "green"
                },
                {
                  "word": "accompany",
                  "ipa": "əˈkʌmpənɪ",
                  "meaning": "v. 陪同，陪伴，",
                  "level": "green"
                },
                {
                  "word": "passage",
                  "ipa": "ˈpæsɪdʒ",
                  "meaning": "n.（文章等的）一节，",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 4,
          "mainIdea": "同行评议结果显示实验组词汇更高级且能感同身受置身文学角色表达其丰富情感",
          "logicRole": "实证数据：实验组词汇更丰富高级且能深入角色内心表达情感 (Target Group Writes in Role)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P4-S1",
              "text": "The peer-reviewed results showed that the target group of pupils drew on a wider vocabulary, used words \"classed as more sophisticated or rarer\", and wrote at greater length.",
              "translation": "经过同行评审的最终研究结果清晰显示：实验目标组的学生调用了明显更为广阔的词汇库，使用了大量“被归类为更为高级、更为罕见”的生动词汇，且写出的文章篇幅更为充实详尽。",
              "vocab": [
                {
                  "word": "peer-reviewed",
                  "ipa": "ˌpɪərrɪˈvjuːd",
                  "meaning": "adj.经同行评议的",
                  "level": "green"
                },
                {
                  "word": "vocabulary",
                  "ipa": "",
                  "meaning": "n.词汇，词汇表",
                  "level": "red"
                },
                {
                  "word": "sophisticated",
                  "ipa": "səˈfɪstɪkeɪtɪd",
                  "meaning": "adj.复杂巧妙的；老练的；在行的",
                  "level": "green"
                },
                {
                  "word": "rarer",
                  "ipa": "'reərə",
                  "meaning": "adj.更稀罕的；更珍贵的（rare 的比较级）",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P4-S2",
              "text": "They also \"appear to be more comfortable writing in role... while [control] group imagine how they themselves would react to being shipwrecked, [target] group put themselves in the shoes of a literary character and express that character's emotion\".",
              "translation": "他们同样“在代入角色写作时显得更加游刃有余自信从容……对照组学生仅仅局限于想象如果是他们自己遭遇海难会作何反应，而实验目标组学生则能够完全设身处地代入文学角色的内心世界（put themselves in shoes of a literary character / identifying with characters），深刻细腻地抒发表达出那个角色的复杂情感。”",
              "vocab": [
                {
                  "word": "appear to be",
                  "ipa": "/əˈpɪə tə biː/",
                  "meaning": "似乎是，看起来是（常用释义：appear to do/be 表示“似乎”）",
                  "level": "purple"
                },
                {
                  "word": "being",
                  "ipa": "ˈbiːɪŋ",
                  "meaning": "n. 生物；存在；本质",
                  "level": "green"
                },
                {
                  "word": "literary",
                  "ipa": "/ˈlɪtərəri/",
                  "meaning": "文字的；书面创作的（常用释义：文学的）",
                  "level": "red"
                },
                {
                  "word": "character",
                  "ipa": "ˈkærəktə(r)",
                  "meaning": "n.性格,品质,特性;人物,角色;字符,(汉)字",
                  "level": "red"
                },
                {
                  "word": "emotion",
                  "ipa": "ɪˈmoʊʃn",
                  "meaning": "n. 情感；情绪",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 5,
          "mainIdea": "对照组依赖棕榈树等“荒岛陈词滥调”暴露出描写描绘能力的贫乏与薄弱，而实验组描写更为广阔细腻",
          "logicRole": "指出对照组依赖陈词滥调反映出描写能力的薄弱 (Weakness in Description)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P5-S1",
              "text": "The Time to Act study also found that while control pupils relied on \"desert island clichés\" such as palm trees, target pupils were \"more expansive [giving] a broader picture of the sky, the sea and the atmospheric conditions\".",
              "translation": "这项名为《行动之时》（Time to Act）的权威研究还发现：对照组学生通篇严重依赖诸如棕榈树等“荒岛陈词滥调”（desert island clichés），暴露出描写景物能力的严重匮乏薄弱（shows their weakness in description）；相比之下，实验目标组学生笔下的描写则显得“更具拓展张力，勾勒出了关于天空、海洋以及大气气象环境的更加广袤深邃的宏伟画卷”。",
              "vocab": [
                {
                  "word": "Act",
                  "ipa": "ækt",
                  "meaning": "v.表演;举动;起作用 n.行为,法令;一幕",
                  "level": "red"
                },
                {
                  "word": "desert",
                  "ipa": "",
                  "meaning": "n.沙漠，荒漠；荒凉的地方；应得的赏罚",
                  "level": "red"
                },
                {
                  "word": "palm",
                  "ipa": "pɑːm",
                  "meaning": "n.手掌，手心；棕榈树 v.把…藏在手中（尤指玩戏法）",
                  "level": "green"
                },
                {
                  "word": "expansive",
                  "ipa": "ɪkˈspænsɪv",
                  "meaning": "adj.广阔的；广泛的；扩张的，发展的；开朗的，健谈的",
                  "level": "green"
                },
                {
                  "word": "picture",
                  "ipa": "/ˈpɪktʃə/",
                  "meaning": "情况；局面（常用释义：图画；照片；整体形势）",
                  "level": "red"
                },
                {
                  "word": "atmospheric",
                  "ipa": "ˌætməsˈferɪk",
                  "meaning": "adj.大气（层）的，有关大气的；富有情调的",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 6,
          "mainIdea": "换位思考体验不同情感能有效培养共情力，证明艺术融入教育具有不可替代的价值",
          "logicRole": "O'Hanlon 强调设身处地换位思考提升情感共情素养 (Identifying with Characters Boosts Literacy)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P6-S1",
              "text": "O'Hanlon said she had been most surprised by the \"emotional literacy that was evident in the [target] children's writing\" and that they were \"more resilient in their writing, more hopeful\".",
              "translation": "O'Hanlon 女士坦言，最令她感到惊艳与震撼的，莫过于“实验组孩子书面表达中所展露出的极其惊人的情感共情素养（emotional literacy）”，他们在字里行间展现出了“更坚韧的内心力量与更满怀希望的情感光芒”。",
              "vocab": [
                {
                  "word": "emotional",
                  "ipa": "ɪˈmoʊʃən(ə)l",
                  "meaning": "adj.情感的；情绪的；激动人心的；情绪激动的",
                  "level": "green"
                },
                {
                  "word": "literacy",
                  "ipa": "ˈlɪtərəsi",
                  "meaning": "n.识字，读写能力",
                  "level": "green"
                },
                {
                  "word": "evident",
                  "ipa": "ˈevɪdənt",
                  "meaning": "adj.清楚的，显而易见",
                  "level": "green"
                },
                {
                  "word": "resilient",
                  "ipa": "rɪˈzɪliənt",
                  "meaning": "adj.有复原力的；有弹性的，有回弹力的",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P6-S2",
              "text": "She added: \"The emotional understanding was very evident and it is probably related to the [rehearsal room process] where you are used to trying to imagine your way through. They were comfortable in describing different emotional states and part of what you do in drama is put yourself in different shoes.\" The study showed the importance of embedding the arts in education, she said.",
              "translation": "她进一步补充道：“这种深刻的情感理解力极其清晰鲜明，它很可能正归功于排练室体验过程——在排练中你早已习惯了通过想象去体悟一切。孩子们能够极其自如地精准描绘各种不同的复杂情绪状态，而戏剧的核心本质正是让你设身处地穿上别人的鞋子换位思考（put yourself in different shoes / identifying with literary characters）。”她由衷感叹道，这项研究雄辩地证明了将艺术深度融入基础教育的极端重要性。",
              "vocab": [
                {
                  "word": "emotional",
                  "ipa": "ɪˈmoʊʃən(ə)l",
                  "meaning": "adj.情感的；情绪的；激动人心的；情绪激动的",
                  "level": "green"
                },
                {
                  "word": "very",
                  "ipa": "",
                  "meaning": "adj.恰好的，正是的",
                  "level": "green"
                },
                {
                  "word": "evident",
                  "ipa": "ˈevɪdənt",
                  "meaning": "adj.清楚的，显而易见",
                  "level": "green"
                },
                {
                  "word": "probably",
                  "ipa": "ˈprɑːbəbli",
                  "meaning": "adv.大概，或许",
                  "level": "red"
                },
                {
                  "word": "related",
                  "ipa": "",
                  "meaning": "v.-ed/a. 相关的，有联系的；有亲戚关系的；",
                  "level": "green"
                },
                {
                  "word": "rehearsal",
                  "ipa": "rɪˈhɜːs(ə)l",
                  "meaning": "n.排练，排演；预演，演习；（~of）复述，重复",
                  "level": "green"
                },
                {
                  "word": "room",
                  "ipa": "ˈweɪtɪŋ- rʊm",
                  "meaning": "n.候诊室,",
                  "level": "green"
                },
                {
                  "word": "process",
                  "ipa": "/ˈprəʊses/",
                  "meaning": "流程；业务流程（常用释义：过程；加工；处理）",
                  "level": "red"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 7,
          "mainIdea": "莎士比亚两万高超词汇结合全身体验，使得该教学法在莎剧中能发挥出最卓越的效果",
          "logicRole": "指出莎翁浩瀚词汇与全身体验使该法在莎剧上效果最为显著 (Method Works Best with Shakespeare)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P7-S1",
              "text": "But could the results be replicated with any old dramatist? O'Hanlon said more research would be needed but suggested that Shakespeare's use of 20,000 words, compared with the everyday 2,000 words, gave a \"massive expansion of language into children's lives\", which was combined with children \"using their whole bodies to bring words to life\".",
              "translation": "然而，这般辉煌的教学成果能否在任何其他古代剧作家的作品中轻易复制？O'Hanlon 表示虽然仍需更多对比实证研究，但她深刻指出：莎士比亚戏剧中庞大浩瀚的20,000个词汇量（相比之下现代人日常词汇仅有可怜的2,000个），给孩子们的语言世界带来了“空前巨大的语言暴涨式拓展扩张”，而这种浩瀚词汇又与孩子们“运用全身肢体去赋予词语鲜活生命（bring words to life）”的沉浸式表演完美结合在了一起（inferred that the new teaching method may work best with Shakespeare）。",
              "vocab": [
                {
                  "word": "dramatist",
                  "ipa": "ˈdræmətɪst",
                  "meaning": "n.剧作家，剧本作者",
                  "level": "green"
                },
                {
                  "word": "use",
                  "ipa": "",
                  "meaning": "n./v. 使用，用途",
                  "level": "green"
                },
                {
                  "word": "massive",
                  "ipa": "ˈmæsɪv",
                  "meaning": "adj.结实的；非常严重的；大量的，大规模的",
                  "level": "green"
                },
                {
                  "word": "expansion",
                  "ipa": "ɪkˈspænʃ(ə)n",
                  "meaning": "n.扩大，扩张；扩充，展开；扩张物；膨胀",
                  "level": "green"
                },
                {
                  "word": "language",
                  "ipa": "ˈlæŋɡwɪdʒ",
                  "meaning": "n. 语言",
                  "level": "green"
                },
                {
                  "word": "lives",
                  "ipa": "/laɪvz/",
                  "meaning": "生活，人生经历（常用释义：life 的复数；生命；生活；一生）",
                  "level": "red"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            }
          ]
        }
      ],
      "questions": [
        {
          "qIndex": 21,
          "type": "细节事实题",
          "tangchiModel": "唐迟细节题模型：抓住第一、二段关于“rehearsal room”（排练室）教学法的核心要求（only if you act him out / drama-based activity / actors work），锁定该教学法要求学生亲自扮演莎剧中的角色（play the roles in Shakespeare）",
          "stem": "The \"rehearsal room\" approach requires pupils to",
          "stemKeywords": [
            "\"rehearsal room\" approach requires pupils to"
          ],
          "targetSentences": [
            "P1-S1",
            "P2-S1",
            "P3-S2"
          ],
          "officialAnswer": "C",
          "options": [
            {
              "key": "A",
              "text": "rewrite the lines from Shakespeare",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 偷换概念】学生是在扮演角色写漂流瓶信件，非重写修改莎士比亚的原著台词台本。",
              "refSentences": [
                "P3-S2"
              ]
            },
            {
              "key": "B",
              "text": "watch RSC actors' performances",
              "isCorrect": false,
              "distractorType": "正反颠倒被动观看",
              "analysis": "【干扰特征: 被动观看偏差】强调学生亲自上台表演扮演（act him out / drama activity），非坐在台下观看演员表演。",
              "refSentences": [
                "P1-S1",
                "P2-S2"
              ]
            },
            {
              "key": "C",
              "text": "play the roles in Shakespeare",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P1-S1 开篇明确限定'But only if you act him out'（只有当你亲自将莎剧表演演绎出来）；P3-S2 进一步交代实验组参与了'30-minute drama-based activity... as Ferdinand after shipwreck'（扮演斐迪南王子的沉浸式戏剧表演）。act him out / drama-based activity 与 play the roles in Shakespeare（扮演莎士比亚戏剧中的角色）完全同义对应。C选项完全精准吻合。",
              "refSentences": [
                "P1-S1",
                "P3-S2"
              ]
            },
            {
              "key": "D",
              "text": "study drama under RSC artists",
              "isCorrect": false,
              "distractorType": "偷换主体",
              "analysis": "【干扰特征: 概念偏差】实验在45所普通小学由一线教师按戏剧法执教，非由 RSC 艺术家亲自手把手授课。",
              "refSentences": [
                "P3-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "act him out + drama-based activity writing as Ferdinand = play the roles in Shakespeare。",
            "methodSummary": "原句细节同义改写：act him out 转换为 play the roles in Shakespeare。"
          }
        },
        {
          "qIndex": 22,
          "type": "细节目的题",
          "tangchiModel": "唐迟目的题模型：抓住第三段将学生分成实验目标组与对照组（target and control groups）的核心实验目的，锁定检验教学指导方式的改变是否能提升学习成效（the change in instruction enhances learning outcomes）",
          "stem": "The study divided the pupils into two groups to find whether",
          "stemKeywords": [
            "divided the pupils into two groups to find whether"
          ],
          "targetSentences": [
            "P3-S1",
            "P3-S2"
          ],
          "officialAnswer": "A",
          "options": [
            {
              "key": "A",
              "text": "the change in instruction enhances learning outcomes",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 实验设计宗旨提炼】P3-S1 和 P3-S2 明确阐述随机对照试验（RCT）的科学逻辑：将未接触过 RSC 教学法的学生随机分为实验组（给予30分钟戏剧教学）与对照组（常规教学），比较两组在词汇、写作和情感理解等各项学习成果上的显著差异。分组实验的核心科研目的正是为了科学检验：教学方式的改变（change in instruction / drama-based approach）是否能够显著提升学生的实际学习成效（enhances learning outcomes）。A选项完全精准吻合实验目的。",
              "refSentences": [
                "P3-S1",
                "P3-S2",
                "P4-S1"
              ]
            },
            {
              "key": "B",
              "text": "expanding vocabulary helps develop reading fluency",
              "isCorrect": false,
              "distractorType": "偷换自变量因变量",
              "analysis": "【干扰特征: 变量颠倒】词汇扩展是戏剧教学带来的结果产物之一，非作为自变量去测试阅读流畅度。",
              "refSentences": [
                "P4-S1"
              ]
            },
            {
              "key": "C",
              "text": "emotion affects understanding of sophisticated works",
              "isCorrect": false,
              "distractorType": "偷换核心变量",
              "analysis": "【干扰特征: 偏离实验主线】实验的核心自变量是是否采用戏剧排练室教学法，非测试情绪对理解复杂作品的影响。",
              "refSentences": [
                "P3-S2",
                "P4-S2"
              ]
            },
            {
              "key": "D",
              "text": "the classroom activity stimulates interest in the arts",
              "isCorrect": false,
              "distractorType": "偏离主旨测量项",
              "analysis": "【干扰特征: 概念偏差】实验核心测量的是读写能力（literacy）、词汇丰富度和情感共情写作，非单纯激发艺术兴趣。",
              "refSentences": [
                "P1-S1",
                "P4-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "target group (with drama activity) vs control group (without) measured on literacy outcomes = whether the change in instruction enhances learning outcomes。",
            "methodSummary": "对照实验逻辑提炼法：识别对照组与实验组的干预变量（instruction）与测量指标（learning outcomes）。"
          }
        },
        {
          "qIndex": 23,
          "type": "推断判断题",
          "tangchiModel": "唐迟推断题模型：抓住第五段对照组学生依赖棕榈树等“荒岛陈词滥调”（desert island clichés）与实验组展现天海大气宏伟画卷的对比，锁定这反映出对照组学生在景物描写与语言表达能力上的薄弱与贫乏（weakness in description）",
          "stem": "Control pupils' reliance on \"desert island clichés\" shows their",
          "stemKeywords": [
            "Control pupils' reliance on \"desert island clichés\" shows their"
          ],
          "targetSentences": [
            "P5-S1"
          ],
          "officialAnswer": "A",
          "options": [
            {
              "key": "A",
              "text": "weakness in description",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P5-S1 明确将对照组与实验组进行对比：'while control pupils relied on \"desert island clichés\" such as palm trees, target pupils were more expansive [giving] a broader picture of the sky, the sea and atmospheric conditions'（对照组依赖棕榈树等荒岛老套陈词滥调，而实验组则拓展描绘出天空大海气象等宏大画卷）。对照组只能写出老套俗套的套话词汇，这极其鲜明地暴露了他们在书面景物描写能力上的严重薄弱与欠缺（weakness in description）。A选项精准吻合。",
              "refSentences": [
                "P5-S1"
              ]
            },
            {
              "key": "B",
              "text": "omission of small details",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 概念偏差】他们写了棕榈树等细节，问题在于用词俗套老旧陈腐，非遗漏小细节。",
              "refSentences": [
                "P5-S1"
              ]
            },
            {
              "key": "C",
              "text": "casual style of writing",
              "isCorrect": false,
              "distractorType": "无中生有",
              "analysis": "【干扰特征: 概念偏差】未提及写作风格随意（casual），核心在于描写功力贫乏。",
              "refSentences": [
                "P5-S1"
              ]
            },
            {
              "key": "D",
              "text": "preference for big words",
              "isCorrect": false,
              "distractorType": "张冠李戴实验组",
              "analysis": "【干扰特征: 主体颠倒】使用高级罕见大词的是实验目标组学生（P4-S1: target group used rarer words），对照组使用的是俗套小词。",
              "refSentences": [
                "P4-S1",
                "P5-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "control relied on clichés vs target giving broader picture of sky and sea = weakness in description。",
            "methodSummary": "对比反差推断：relied on clichés 暴露出写作描写能力上的欠缺（weakness in description）。"
          }
        },
        {
          "qIndex": 24,
          "type": "观点细节题",
          "tangchiModel": "唐迟观点题模型：抓住第六段 O'Hanlon 解释提升儿童情感共情素养的核心机制（put yourself in different shoes / put themselves in shoes of a literary character and express emotion），锁定与文学角色建立深度共鸣同理心（Identifying with literary characters）",
          "stem": "What can promote children's emotional literacy according to O'Hanlon?",
          "stemKeywords": [
            "promote children's emotional literacy according to O'Hanlon"
          ],
          "targetSentences": [
            "P4-S2",
            "P6-S2"
          ],
          "officialAnswer": "B",
          "options": [
            {
              "key": "A",
              "text": "Writing in an imaginative manner.",
              "isCorrect": false,
              "distractorType": "以偏概全过于笼统",
              "analysis": "【干扰特征: 过于宽泛】对照组也在发挥想象（imagine react to shipwreck），关键在于是否设身处地代入文学角色内心。",
              "refSentences": [
                "P4-S2",
                "P6-S2"
              ]
            },
            {
              "key": "B",
              "text": "Identifying with literary characters.",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P4-S2 和 P6-S2 明确阐述促进情感素养的核心秘诀：'put themselves in the shoes of a literary character and express that character's emotion... part of what you do in drama is put yourself in different shoes'（设身处地代入文学角色的内心世界并抒发其情感，戏剧的本质正是让你穿上别人的鞋子换位思考共情）。put themselves in the shoes of a literary character 与 Identifying with literary characters（认同并与文学角色产生深度情感共鸣/同理心）完全同义对应。B选项完全精准吻合。",
              "refSentences": [
                "P4-S2",
                "P6-S2"
              ]
            },
            {
              "key": "C",
              "text": "Drawing inspiration from nature.",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 偷换源头】灵感来自莎剧文学文本与角色扮演，非从大自然中汲取灵感。",
              "refSentences": [
                "P6-S2"
              ]
            },
            {
              "key": "D",
              "text": "Concentrating on real-life situations.",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实相反】对照组局限于自身现实反应，实验组跳出真实自我进入虚构文学角色世界（literary character）。",
              "refSentences": [
                "P4-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "put yourself in the shoes of a literary character = Identifying with literary characters（与文学角色产生共情同理心）。",
            "methodSummary": "习语转换法：put oneself in someone's shoes 直接转换为 identify with。"
          }
        },
        {
          "qIndex": 25,
          "type": "推断判断题",
          "tangchiModel": "唐迟推断题模型：抓住第七段 O'Hanlon 针对该教学法能否应用于其他剧作家的回答（Shakespeare's 20,000 words gave massive expansion of language combined with using whole body），锁定该全新教学法在莎士比亚戏剧上能够发挥出最卓越的效果（may work best with Shakespeare）",
          "stem": "It can be inferred from the last paragraph that",
          "stemKeywords": [
            "inferred from the last paragraph"
          ],
          "targetSentences": [
            "P7-S1"
          ],
          "officialAnswer": "A",
          "options": [
            {
              "key": "A",
              "text": "the new teaching method may work best with Shakespeare",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 合理推断】P7-S1 面对“该成果能否在其他古代剧作家身上复制”的设问，O'Hanlon 深入分析指出：莎士比亚独有的20,000个海量词汇给儿童带来了巨大的语言扩张，结合全身体验赋予词语生命。由此可严密推断出：正是莎剧独一无二的浩瀚词汇库与戏剧张力，使得这种沉浸式排练室教学法在莎士比亚作品上能够发挥出最为显著卓越的最佳成效（may work best with Shakespeare）。A选项精准契合推理。",
              "refSentences": [
                "P7-S1"
              ]
            },
            {
              "key": "B",
              "text": "the language of Shakespeare may be formidable for pupils",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 褒贬颠倒】全篇强调莎剧语言给九至十岁儿童带来了巨大的语言能力腾飞，绝非令人望而生畏难以逾越（formidable）。",
              "refSentences": [
                "P4-S1",
                "P7-S1"
              ]
            },
            {
              "key": "C",
              "text": "other old dramatists may be included in primary education",
              "isCorrect": false,
              "distractorType": "过度推断",
              "analysis": "【干扰特征: 概念偏差】O'Hanlon 表示其他剧作家尚需更多实证研究，并未断言应将其纳入小学课程。",
              "refSentences": [
                "P7-S1"
              ]
            },
            {
              "key": "D",
              "text": "pupils may be reluctant to work on other old dramatists",
              "isCorrect": false,
              "distractorType": "无中生有",
              "analysis": "【干扰特征: 无端捏造】未提及学生不愿意学习其他古代剧作家。",
              "refSentences": [
                "P7-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "Shakespeare's 20,000 words vs 2,000 everyday words combined with whole body acting = the new teaching method may work best with Shakespeare。",
            "methodSummary": "末段独特优势推断：抓准莎翁两万词汇的独特结合点，推导出 work best with Shakespeare。"
          }
        }
      ]
    },
    {
      "id": "text2",
      "number": 2,
      "title": "Carbon Emissions from Scientific Research: A Necessary Price for Global Sustainability",
      "chineseTitle": "科研算力碳排放的正当性与绿色可持续计算愿景",
      "topic": "气候变化与绿色科研计算 / 科学家欲缩减科研应对碳排、科研碳排从长远看具有正当性、芬兰 LUMI 超算范例与高能耗科研的必然性",
      "overview": "本文探讨了科学研究自身的高能耗与碳排放争议。作者开篇对部分科学家企图通过缩减科研规模来降低碳排的意图表示极大震惊，客观指出实验室、卫星和数据中心确实消耗巨大电能。然而作者深刻论证指出：这是人类认知理解客观世界所必须付出的崇高代价，科技研发的碳排完全物有所值，从长远来看完全具有正当合理性。文章以格林教授发明 PERC 太阳能电池荣获千禧年奖为例展示科学探索带来的巨大减排回报，并盛赞芬兰 LUMI 超级计算机依靠水电驱动、余热供暖实现负碳排，是绿色可持续计算的杰出典范，大声疾呼高能耗科研不可避免，实现净零排放必须大力打造更多绿色可持续超算。",
      "paragraphs": [
        {
          "pIndex": 1,
          "mainIdea": "作者对部分科学家因企图降低碳排放而打算缩减科研建模规模的意图感到极其震惊",
          "logicRole": "提出部分科学家试图缩减科研规模以减排的令人震惊动向 (Shocked by Intention to Reduce Research)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P1-S1",
              "text": "I was shocked to learn recently that some scientists want to scale back their research in an effort to decrease carbon emissions.",
              "translation": "我最近获悉某些科学家竟然企图通过缩减削减自身的科学研究规模（scale back their research / intention to reduce their research）来降低碳排放，对此我感到极度震惊（expressed great surprise）！",
              "vocab": [
                {
                  "word": "scale",
                  "ipa": "/skeɪl/",
                  "meaning": "量表；测量等级体系。（常用释义：刻度；规模；比例；等级。）",
                  "level": "red"
                },
                {
                  "word": "decrease",
                  "ipa": "dɪˈkriːs",
                  "meaning": "v.（使）减少，（使）降低 n.减少，降低",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            },
            {
              "sIndex": 2,
              "id": "P1-S2",
              "text": "The crisis is here, they said, and we need to cut back on our energy-intensive modelling. At the very least, we need to make our energy use far more sustainable.",
              "translation": "他们声称：气候危机迫在眉睫，我们必须立刻削减自身那些高能耗的计算机模型模拟运算；退一万步讲，我们也必须让自身的能源消耗变得远为更加绿色可持续。",
              "vocab": [
                {
                  "word": "crisis",
                  "ipa": "ˈkraɪsɪs",
                  "meaning": "n.危机；关键时刻；转折点 adj.用于处理危机的",
                  "level": "green"
                },
                {
                  "word": "modelling",
                  "ipa": "ˈmɒdəlɪŋ",
                  "meaning": "n.模化；建模 v.模仿；做模型（model 的现在分词形式）",
                  "level": "green"
                },
                {
                  "word": "very",
                  "ipa": "",
                  "meaning": "adj.恰好的，正是的",
                  "level": "green"
                },
                {
                  "word": "energy",
                  "ipa": "ˈɛnərdʒi",
                  "meaning": "n. 能量；精力；活力",
                  "level": "green"
                },
                {
                  "word": "use",
                  "ipa": "",
                  "meaning": "n./v. 使用，用途",
                  "level": "green"
                },
                {
                  "word": "sustainable",
                  "ipa": "səˈsteɪnəb(ə)l",
                  "meaning": "adj.可持续的，持续性的",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 2,
          "mainIdea": "实验室、火箭和数据中心确实消耗巨大能源，耗电已占全球1%",
          "logicRole": "客观承认科研与数据中心确实属于高能耗领域 (Energy-Intensive Research Reality)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P2-S1",
              "text": "It is unarguable that our laboratories, scientific instruments, rockets and satellites—the tools we scientists need to measure the planet's pulse—demand significant amounts of energy both in their construction and operation.",
              "translation": "毋庸置疑，我们的前沿实验室、精密科研仪器、运载火箭与人造卫星——这些我们科学家用来把脉地球跳动节律所不可或缺的利器——在其建造制造与日常运转过程中确实都需要消耗海量的巨大能源（Energy-intensive research work is inevitable）。",
              "vocab": [
                {
                  "word": "unarguable",
                  "ipa": "ʌnˈɑːɡjuəbl",
                  "meaning": "adj.[法] 无可争辩的；不可论证的",
                  "level": "green"
                },
                {
                  "word": "measure",
                  "ipa": "/ˈmeʒə/",
                  "meaning": "衡量；测量。（常用释义：测量；衡量；尺寸为。）",
                  "level": "red"
                },
                {
                  "word": "pulse",
                  "ipa": "pʌls",
                  "meaning": "n.脉搏，脉率；律动；脉冲 v.（有节奏地）搏动，跳动；洋溢着，",
                  "level": "green"
                },
                {
                  "word": "significant",
                  "ipa": "sɪɡˈnɪfɪkənt",
                  "meaning": "adj.重要的；显著的；意味深长的",
                  "level": "green"
                },
                {
                  "word": "energy",
                  "ipa": "ˈɛnərdʒi",
                  "meaning": "n. 能量；精力；活力",
                  "level": "green"
                },
                {
                  "word": "construction",
                  "ipa": "/kənˈstrʌkʃn/",
                  "meaning": "建设；建筑业（常用释义：建造；结构；解释）",
                  "level": "red"
                },
                {
                  "word": "operation",
                  "ipa": "ˌɒpərˈeɪʃən",
                  "meaning": "n. 操作；经营；手术",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            },
            {
              "sIndex": 2,
              "id": "P2-S2",
              "text": "And it is equally true that science's unrelenting appetite for information has caused a mushrooming of energy-intensive data centres around the world. According to the International Energy Agency, these buildings now consume about 1 per cent of the world's electricity.",
              "translation": "同样不可否认的是，科学界对数据的无尽渴求导致全球各地高能耗数据中心如雨后春笋般疯狂涌现。根据国际能源署（IEA）的权威统计，这些建筑设施目前消耗了全球整整约1%的总电力供应。",
              "vocab": [
                {
                  "word": "according to",
                  "ipa": "/əˈkɔːdɪŋ tuː/",
                  "meaning": "根据；按照……的说法（常用释义：用于交代观点来源）",
                  "level": "purple"
                },
                {
                  "word": "equally",
                  "ipa": "ˈiːkwəli",
                  "meaning": "adv.相等地；平等地；同样（重要）地",
                  "level": "green"
                },
                {
                  "word": "unrelenting",
                  "ipa": "ˌʌnrɪˈlentɪŋ",
                  "meaning": "adj.无情的；不屈不挠的；不松懈的",
                  "level": "green"
                },
                {
                  "word": "appetite",
                  "ipa": "ˈæpɪtaɪt",
                  "meaning": "n.食欲，胃口；欲望，爱好",
                  "level": "green"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "information",
                  "ipa": "ɪnfəˈmeɪʃ(ə)n",
                  "meaning": "n. 信息",
                  "level": "green"
                },
                {
                  "word": "mushrooming",
                  "ipa": "mʌʃrumiŋ",
                  "meaning": "v.迅速增长；呈蘑菇状升腾；（mushroom 的现在分词）n.迅",
                  "level": "green"
                },
                {
                  "word": "data",
                  "ipa": "/ˈdeɪtə/ or /ˈdɑːtə/",
                  "meaning": "数据，资料（常用释义：数据；资料）",
                  "level": "red"
                },
                {
                  "word": "around",
                  "ipa": "əˈraʊnd",
                  "meaning": "adv.存在着",
                  "level": "red"
                },
                {
                  "word": "International",
                  "ipa": "ɪntəˈnæʃən(ə)l",
                  "meaning": "adj. 国际",
                  "level": "green"
                },
                {
                  "word": "Energy",
                  "ipa": "ˈɛnərdʒi",
                  "meaning": "n. 能量；精力；活力",
                  "level": "green"
                },
                {
                  "word": "Agency",
                  "ipa": "/ˈeɪdʒənsi/",
                  "meaning": "机构，尤指资助机构（常用释义：机关；代理处；专门部门）",
                  "level": "red"
                },
                {
                  "word": "consume",
                  "ipa": "kənˈsʌltənt",
                  "meaning": "v. 消耗，耗费（燃",
                  "level": "green"
                },
                {
                  "word": "about",
                  "ipa": "əˈbaʊt",
                  "meaning": "prep.从事于,忙于",
                  "level": "red"
                },
                {
                  "word": "electricity",
                  "ipa": "ɪˌlɛkˈtrɪsɪti",
                  "meaning": "n. 电；电力",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 3,
          "mainIdea": "科研产生的碳排放是从长远拯救地球必须付出的代价，从长远来看完全是正当合理的",
          "logicRole": "核心论点：科研碳排从长远看具有正当性，是理解世界拯救地球的必要代价 (Carbon Emissions are Justifiable in Long Run)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P3-S1",
              "text": "However, this is a price we must pay for understanding the world.",
              "translation": "然而，这恰恰是我们为了深刻认知理解客观世界所必须付出的必要代价！",
              "vocab": [
                {
                  "word": "However",
                  "ipa": "/haʊˈevə/",
                  "meaning": "然而、不过（常用释义：无论如何）",
                  "level": "red"
                },
                {
                  "word": "pay",
                  "ipa": "peɪ",
                  "meaning": "n. 工资，报",
                  "level": "green"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P3-S2",
              "text": "How can we inform decision makers about the best ways to bring down carbon emissions if we can't track the amount of carbon dioxide in the atmosphere, where it's coming from and who's producing it? The carbon emissions from technological research are well spent: ultimately this research will safeguard the future of our planet.",
              "translation": "如果我们甚至无法精准追踪大气中二氧化碳的确切存量、来源路径以及排放责任方，我们又究竟如何向政策制定者提供关于减少碳排放的最佳施策建议？科技研发所产生的碳排放完全是物有所值的崇高投入（well spent）——从根本长远来看，这些科研成果将真正捍卫守护我们整个人类地球的未来命运（carbon emissions are justifiable in the long run）！",
              "vocab": [
                {
                  "word": "inform",
                  "ipa": "ɪnˈfɔːm",
                  "meaning": "v.通知，告知；检举（~on）；渗透；了解，熟悉；影响",
                  "level": "green"
                },
                {
                  "word": "decision",
                  "ipa": "/dɪˈsɪʒən/",
                  "meaning": "法院裁决、判决（常用释义：决定；决心）",
                  "level": "red"
                },
                {
                  "word": "about",
                  "ipa": "əˈbaʊt",
                  "meaning": "prep.从事于,忙于",
                  "level": "red"
                },
                {
                  "word": "down",
                  "ipa": "daʊn",
                  "meaning": "v.使倒下,击倒",
                  "level": "red"
                },
                {
                  "word": "track",
                  "ipa": "/træk/",
                  "meaning": "轨迹；路径（常用释义：轨道；小路；痕迹；发展路线）",
                  "level": "red"
                },
                {
                  "word": "amount",
                  "ipa": "əˈmaʊnt",
                  "meaning": "n./v. 金额，数量，",
                  "level": "green"
                },
                {
                  "word": "dioxide",
                  "ipa": "daɪˈɒksaɪd",
                  "meaning": "n. 二氧化物",
                  "level": "green"
                },
                {
                  "word": "atmosphere",
                  "ipa": "/ˈætməsfɪə(r)/",
                  "meaning": "氛围；气氛（常用释义：大气层；环境气氛）",
                  "level": "green"
                },
                {
                  "word": "technological",
                  "ipa": "ˌteknəˈlɑːdʒɪk(ə)l",
                  "meaning": "adj.技术的；科技的",
                  "level": "green"
                },
                {
                  "word": "ultimately",
                  "ipa": "ˈʌltɪmətli",
                  "meaning": "adv. 最终；根本上",
                  "level": "green"
                },
                {
                  "word": "will",
                  "ipa": "/wɪl/",
                  "meaning": "遗嘱（常用释义：将要；意志；遗嘱）",
                  "level": "green"
                },
                {
                  "word": "safeguard",
                  "ipa": "ˈseɪfɡɑːrd",
                  "meaning": "v.保护；保卫 n.保障条款，保护措施",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 4,
          "mainIdea": "展现科学的巨大效力对于解决人类重大挑战至关重要",
          "logicRole": "强调向公众证明科学解决人类挑战巨大效力的重要性 (Demonstrating Efficacy of Science)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P4-S1",
              "text": "It can be hard for scientists to make the case because our work is complex, often takes place behind closed doors and does not always lend itself to easy interpretation or explanation.",
              "translation": "对于科学家而言，要向外界充分阐明这一道理往往颇具难度，因为我们的工作极其高深复杂，通常在密闭实验室中进行，并不总是容易被通俗解读或向大众解释清楚。",
              "vocab": [
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "case",
                  "ipa": "keɪs",
                  "meaning": "n.事例，案",
                  "level": "green"
                },
                {
                  "word": "work",
                  "ipa": "wɜ:rk",
                  "meaning": "n.工作(量);作品;[pl.]工厂 v.工作;运转",
                  "level": "red"
                },
                {
                  "word": "complex",
                  "ipa": "/ˈkɒmpleks/",
                  "meaning": "复杂的，多层次的（常用释义：复杂的；复合的；综合体）",
                  "level": "red"
                },
                {
                  "word": "place",
                  "ipa": "",
                  "meaning": "v.放置",
                  "level": "green"
                },
                {
                  "word": "interpretation",
                  "ipa": "ɪnˌtɜːprəˈteɪʃ(ə)n",
                  "meaning": "n.解释，理解；表演，演奏，艺术处理；口译",
                  "level": "green"
                },
                {
                  "word": "explanation",
                  "ipa": "ˌekspləˈneɪʃ(ə)n",
                  "meaning": "n.解释，说明；说明性文字；理由，原因",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P4-S2",
              "text": "But demonstrating the efficacy of science will be crucial if we are to solve humanity's greatest challenges.",
              "translation": "但如果我们真正渴望解决全人类面临的最严峻重大挑战，有力彰显并证明科学探索的巨大效力将是绝对至关重要的。",
              "vocab": [
                {
                  "word": "will",
                  "ipa": "/wɪl/",
                  "meaning": "遗嘱（常用释义：将要；意志；遗嘱）",
                  "level": "green"
                },
                {
                  "word": "crucial",
                  "ipa": "/ˈkruːʃəl/",
                  "meaning": "至关重要的、决定性的（常用释义：极其重要的）",
                  "level": "purple"
                },
                {
                  "word": "humanity",
                  "ipa": "/hjuːˈmænəti/",
                  "meaning": "人类（常用释义：人性；人道）",
                  "level": "red"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 5,
          "mainIdea": "设立千禧年科技奖表彰人类创造力，格林教授发明高效太阳能电池正是科研探索带来丰硕回报的杰出典范",
          "logicRole": "以格林教授获千禧年奖为例展示科学探索与创新带来的巨大丰硕回报 (Rewards of Scientific Endeavours)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P1-S1",
              "text": "Recognising the hope that science and engineering can bring was the impetus behind the creation of the Millennium Technology Prize, which is now entering its 20th year as a celebration of human ingenuity.",
              "translation": "深刻体认科学与工程所能带来的光明希望，正是设立‘千禧年科技奖’的强大初衷动力——如今该奖项已跨入第20个年头，成为颂扬全人类非凡创造智慧的崇高盛典。",
              "vocab": [
                {
                  "word": "impetus",
                  "ipa": "ˈɪmpɪtəs",
                  "meaning": "n.动力，促进；动量，冲力",
                  "level": "green"
                },
                {
                  "word": "Millennium",
                  "ipa": "mɪˈleniəm",
                  "meaning": "n.一千年；千周年纪念日，千禧年（the~）",
                  "level": "green"
                },
                {
                  "word": "celebration",
                  "ipa": "selɪˈbreɪʃ(ə)n",
                  "meaning": "n. 庆祝；庆",
                  "level": "green"
                },
                {
                  "word": "ingenuity",
                  "ipa": "ˌɪndʒəˈnjuːəti",
                  "meaning": "n.心灵手巧，聪明才智，独创力",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P5-S2",
              "text": "One of the past winners, Professor Martin Green from the University of New South Wales, Australia, is the inventor of the Passivated Emitter and Rear Cell technology which is now found in most of the world's solar panels. Thanks to his invention, we have a real chance to decrease the world's carbon emissions.",
              "translation": "该奖项的往届得主之一、澳大利亚新南威尔士大学的马丁·格林教授，正是当今全球绝大多数太阳能电池板中广泛应用的“钝化发射极及背部接触”（PERC）太阳能技术的伟大发明者。正是得益于他的这项科学发明，我们今天才真正拥有了大幅降低全球碳排放的切实胜算与丰硕回报（the rewards of scientific endeavours）！",
              "vocab": [
                {
                  "word": "One",
                  "ipa": "/wʌn/",
                  "meaning": "其中一个（问题）（常用释义：一；一个人/事物）",
                  "level": "red"
                },
                {
                  "word": "Professor",
                  "ipa": "prəˈfesə(r)",
                  "meaning": "n. 教授",
                  "level": "green"
                },
                {
                  "word": "New",
                  "ipa": "",
                  "meaning": "adj.新的",
                  "level": "green"
                },
                {
                  "word": "inventor",
                  "ipa": "ɪnˈventə(r)",
                  "meaning": "n.发明者，发明家",
                  "level": "green"
                },
                {
                  "word": "Passivated",
                  "ipa": "ˈpæsɪveɪtɪd",
                  "meaning": "adj.钝化的 v.钝化（passivate 的过去分词）",
                  "level": "green"
                },
                {
                  "word": "Emitter",
                  "ipa": "ɪˈmɪtə(r)",
                  "meaning": "n.发射器，发射体",
                  "level": "green"
                },
                {
                  "word": "Rear",
                  "ipa": "rɪə(r)",
                  "meaning": "n.后部，背部（the~）；后方；臀部，屁股 adj.后方的，背部的 v.",
                  "level": "green"
                },
                {
                  "word": "Cell",
                  "ipa": "sel",
                  "meaning": "n.（监狱的）单人牢房；（修",
                  "level": "green"
                },
                {
                  "word": "invention",
                  "ipa": "/ɪnˈvenʃən/",
                  "meaning": "发明；所谓发明成果（常用释义：发明；创造；虚构）",
                  "level": "green"
                },
                {
                  "word": "decrease",
                  "ipa": "dɪˈkriːs",
                  "meaning": "v.（使）减少，（使）降低 n.减少，降低",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 6,
          "mainIdea": "欧洲最大超算芬兰 LUMI 运用水力发电并将废热供暖城镇实现负碳排，是绿色可持续计算的杰出典范",
          "logicRole": "以芬兰 LUMI 超算负碳排为例展示绿色可持续计算的典范力量 (LUMI: Model of Sustainability)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P6-S1",
              "text": "Every day, scientists, technologists and engineers are discovering new ways to exploit renewable energy sources and develop techniques not just to use power more intelligently but to power our intelligence.",
              "translation": "每一天，广大科学家、技术专家与工程师都在不断探寻利用可再生能源的全新途径，并开发先进技术——不仅为了更聪明智能地使用电力，更是为了用清洁能源来赋能我们的人类智慧。",
              "vocab": [
                {
                  "word": "new",
                  "ipa": "",
                  "meaning": "adj.新的",
                  "level": "green"
                },
                {
                  "word": "exploit",
                  "ipa": "ɪkˈsplɔɪt",
                  "meaning": "v.剥削，压榨；利用；运用，发挥；开发，开拓n.壮举",
                  "level": "green"
                },
                {
                  "word": "renewable",
                  "ipa": "rɪˈnjuːəbl",
                  "meaning": "adj. 可再生的",
                  "level": "green"
                },
                {
                  "word": "energy",
                  "ipa": "ˈɛnərdʒi",
                  "meaning": "n. 能量；精力；活力",
                  "level": "green"
                },
                {
                  "word": "use",
                  "ipa": "",
                  "meaning": "n./v. 使用，用途",
                  "level": "green"
                },
                {
                  "word": "power",
                  "ipa": "",
                  "meaning": "n. 力量，权力；v.推动，驱动",
                  "level": "green"
                },
                {
                  "word": "intelligently",
                  "ipa": "ɪnˈtelɪdʒəntli",
                  "meaning": "adv.聪明地，明智地",
                  "level": "green"
                },
                {
                  "word": "intelligence",
                  "ipa": "ɪnˈtɛlɪdʒəns",
                  "meaning": "n. 智力；情报；理解力",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P6-S2",
              "text": "A great example of this is Europe's largest supercomputer, LUMI in Finland, which is astonishingly carbon negative. Established in an old paper mill, it is powered by a nearby river and its remote heat warms the people who live in the surrounding town of Kajaani.",
              "translation": "这其中最卓越的典范，莫过于坐落于芬兰的欧洲最大超级计算机 LUMI——它令人惊叹地实现了“负碳排放”（carbon negative）。该超算中心设于一座旧造纸厂内，全部依靠邻近河流的水力清洁能源驱动，而其散发的余热更是直接为周边卡亚尼小镇的居民提供市政供暖（LUMI is a model of sustainability efforts）！",
              "vocab": [
                {
                  "word": "astonishingly",
                  "ipa": "əˈstɒnɪʃɪŋli",
                  "meaning": "adv.令人惊讶地",
                  "level": "green"
                },
                {
                  "word": "negative",
                  "ipa": "",
                  "meaning": "adj.消极的；负面的；否定的；坏的；有害的；缺乏热情的；含有否定词的；结果",
                  "level": "red"
                },
                {
                  "word": "Established",
                  "ipa": "ɪˈstæblɪʃt",
                  "meaning": "adj.地位稳固的",
                  "level": "red"
                },
                {
                  "word": "mill",
                  "ipa": "mɪl",
                  "meaning": "n.磨坊，磨粉厂；工厂，制造厂 v.碾碎，磨成粉；（一大群人）闲",
                  "level": "green"
                },
                {
                  "word": "remote",
                  "ipa": "rɪˈməʊt",
                  "meaning": "adj. 偏远的，偏僻的",
                  "level": "green"
                },
                {
                  "word": "surrounding",
                  "ipa": "səˈraʊndɪŋ",
                  "meaning": "adj. 周围的；附近的 n.周围环境（surroundings）v.围绕",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 7,
          "mainIdea": "若要实现净零排放宏伟目标，必须大力推广绿色可持续计算并打造更多 LUMI 超算",
          "logicRole": "呼吁大力发展可持续计算以实现全球净零目标 (Delivering Sustainable Computing)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P7-S1",
              "text": "If the world is to meet its net-zero ambitions, we must think hard about how we can deliver sustainable computing and deliver more LUMIs.",
              "translation": "如果全球真正渴望实现‘净零排放’的宏伟抱负，我们就必须深谋远虑地全力思考：我们该如何打造真正绿色可持续的算力体系，并建造复制出更多像 LUMI 这样卓越非凡的绿色超算中心！",
              "vocab": [
                {
                  "word": "net-zero",
                  "ipa": "'net 'zɪərəʊ",
                  "meaning": "adj. 净零的（指温室气体排放量为零）",
                  "level": "green"
                },
                {
                  "word": "about",
                  "ipa": "əˈbaʊt",
                  "meaning": "prep.从事于,忙于",
                  "level": "red"
                },
                {
                  "word": "sustainable",
                  "ipa": "səˈsteɪnəb(ə)l",
                  "meaning": "adj.可持续的，持续性的",
                  "level": "green"
                },
                {
                  "word": "computing",
                  "ipa": "kəmˈpjuːtɪŋ",
                  "meaning": "n.计算机应用；计算机技术 v.估算，计算（compute 的现",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            }
          ]
        }
      ],
      "questions": [
        {
          "qIndex": 26,
          "type": "情感态度题",
          "tangchiModel": "唐迟观点题模型：抓住第一段首句作者对部分科学家意图的震惊反应（I was shocked to learn recently that some scientists want to scale back their research to decrease emissions），锁定作者对部分科学家企图缩减削减科研规模的意图感到极其震惊与意外",
          "stem": "The author expressed great surprise at some scientists'",
          "stemKeywords": [
            "author expressed great surprise at some scientists'"
          ],
          "targetSentences": [
            "P1-S1"
          ],
          "officialAnswer": "B",
          "options": [
            {
              "key": "A",
              "text": "unwillingness to cut carbon emissions",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实相反】这些科学家正是过于激进地渴望减少碳排放，甚至不惜因此自断手臂削减科研。",
              "refSentences": [
                "P1-S1"
              ]
            },
            {
              "key": "B",
              "text": "intention to reduce their research",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P1-S1 开篇第一句明确点题：'I was shocked to learn recently that some scientists want to scale back their research in an effort to decrease carbon emissions'（我最近震惊地获悉，某些科学家为了降低碳排放居然想缩减削减自身的科学研究）。shocked（感到震惊）与 expressed great surprise 完全同义；want to scale back their research 与 intention to reduce their research（缩减其科研工作的意图）完全同义对应。B选项完全精准吻合。",
              "refSentences": [
                "P1-S1"
              ]
            },
            {
              "key": "C",
              "text": "suspicions about sustainable energy",
              "isCorrect": false,
              "distractorType": "无中生有",
              "analysis": "【干扰特征: 概念偏差】未提及对可持续能源怀有怀疑态度。",
              "refSentences": [
                "P1-S2"
              ]
            },
            {
              "key": "D",
              "text": "waste of electricity in their projects",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 概念偏差】科研用电是探索世界的必要投入，非盲目浪费电能（waste of electricity）。",
              "refSentences": [
                "P2-S1",
                "P3-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "I was shocked to learn scientists want to scale back their research = expressed great surprise at intention to reduce their research。",
            "methodSummary": "首句题眼提取：shocked 对应 expressed great surprise，scale back research 转换为 reduce research。"
          }
        },
        {
          "qIndex": 27,
          "type": "观点细节题",
          "tangchiModel": "唐迟观点题模型：抓住第三段作者对科研产生的碳排放的根本定性（price we must pay for understanding world / well spent: ultimately safeguard future of planet），锁定科研碳排放从长远来看完全是必要且正当合理的（justifiable in the long run）",
          "stem": "The author believes that carbon emissions from research",
          "stemKeywords": [
            "author believes",
            "carbon emissions from research"
          ],
          "targetSentences": [
            "P3-S1",
            "P3-S2"
          ],
          "officialAnswer": "D",
          "options": [
            {
              "key": "A",
              "text": "have caused grave consequences",
              "isCorrect": false,
              "distractorType": "褒贬颠倒",
              "analysis": "【干扰特征: 贬义偏差】作者强调科研碳排带来了长远拯救地球的巨大红利，绝非造成严重恶果。",
              "refSentences": [
                "P3-S2"
              ]
            },
            {
              "key": "B",
              "text": "have aroused groundless worries",
              "isCorrect": false,
              "distractorType": "过度推断",
              "analysis": "【干扰特征: 概念偏差】高能耗是客观事实，作者并非批评担忧毫无根据，而是论证这笔投入从长远看完全物有所值。",
              "refSentences": [
                "P2-S1",
                "P3-S2"
              ]
            },
            {
              "key": "C",
              "text": "are hard to handle at present",
              "isCorrect": false,
              "distractorType": "无中生有",
              "analysis": "【干扰特征: 偏离主线】文章后文展示了芬兰超算等成熟解决途径，非当前难以处理。",
              "refSentences": [
                "P6-S2"
              ]
            },
            {
              "key": "D",
              "text": "are justifiable in the long run",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P3-S1 和 P3-S2 明确阐述作者的核心论断：'this is a price we must pay for understanding the world... The carbon emissions from technological research are well spent: ultimately this research will safeguard the future of our planet'（这是我们认知世界必须付出的代价，科技研发的碳排放完全物有所值，终将拯救守护地球未来）。well spent / price we must pay / safeguard future 与 are justifiable in the long run（从长远来看是完全正当合理的）完全同义对应。D选项精准吻合。",
              "refSentences": [
                "P3-S1",
                "P3-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "price we must pay + well spent + safeguard future of planet = are justifiable in the long run。",
            "methodSummary": "核心观点提炼：price must pay / well spent 抽象概括为 justifiable in the long run。"
          }
        },
        {
          "qIndex": 28,
          "type": "例证题",
          "tangchiModel": "唐迟例证题模型：第五段引用格林教授发明高效太阳能电池荣获千禧年科技奖（PERC technology in most solar panels... real chance to decrease emissions），是为了展示前文论点——科学探索与技术创造能给人类带来巨大的丰硕成果与回报（the rewards of scientific endeavours）",
          "stem": "The example of Green in Paragraph 5 is used to illustrate",
          "stemKeywords": [
            "example of Green in Paragraph 5",
            "used to illustrate"
          ],
          "targetSentences": [
            "P4-S2",
            "P5-S1",
            "P5-S2"
          ],
          "officialAnswer": "C",
          "options": [
            {
              "key": "A",
              "text": "the achievements of great scientists",
              "isCorrect": false,
              "distractorType": "以偏概全人物成就",
              "analysis": "【干扰特征: 局限于个人】举例的宏观论证目的是证明‘科学探索投入能够带来巨大的环境与减排丰硕回报’，非单纯颂扬个人功绩。",
              "refSentences": [
                "P5-S1",
                "P5-S2"
              ]
            },
            {
              "key": "B",
              "text": "the urgency of addressing climate change",
              "isCorrect": false,
              "distractorType": "偷换论点",
              "analysis": "【干扰特征: 偏离论点】案例展示的是科研成果带来的解决胜算与红利，非强调应对危机的紧迫性。",
              "refSentences": [
                "P5-S2"
              ]
            },
            {
              "key": "C",
              "text": "the rewards of scientific endeavours",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 例证论点对应】P4-S2 提出论点'demonstrating the efficacy of science will be crucial if we are to solve humanity's greatest challenges'（证明科学的巨大效力对于解决人类挑战至关重要）；P5-S1 和 P5-S2 举出千禧年科技奖得主格林教授发明 PERC 太阳能电池大幅降低全球碳排的实证。引用该案例正是为了生动论证：坚定的科学探索努力必将给全人类带来极其丰硕的伟大回报与减排红利（the rewards of scientific endeavours）。C选项完全精准对应论点。",
              "refSentences": [
                "P4-S2",
                "P5-S1",
                "P5-S2"
              ]
            },
            {
              "key": "D",
              "text": "the value of fostering human ingenuity",
              "isCorrect": false,
              "distractorType": "次要修饰细节",
              "analysis": "【干扰特征: 次要背景】celebration of human ingenuity 是千禧年科技奖的仪式修饰语，案例核心落在格林发明带来的实质减排回报上。",
              "refSentences": [
                "P5-S1",
                "P5-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "Green invented PERC solar cells which gives real chance to decrease world emissions = the rewards of scientific endeavours（科学探索的丰硕回报）。",
            "methodSummary": "例证题前段论点锁定：demonstrating efficacy of science 结合太阳能减排巨大成果，提炼为 the rewards of scientific endeavours。"
          }
        },
        {
          "qIndex": 29,
          "type": "细节事实题",
          "tangchiModel": "唐迟细节题模型：抓住第六段关于芬兰超级计算机 LUMI 的事实特征（astonishingly carbon negative / powered by river / heat warms town），锁定 LUMI 是绿色可持续发展努力的杰出典范（is a model of sustainability efforts）",
          "stem": "It can be learned from the last two paragraphs that LUMI",
          "stemKeywords": [
            "learned from the last two paragraphs",
            "LUMI"
          ],
          "targetSentences": [
            "P6-S2",
            "P7-S1"
          ],
          "officialAnswer": "A",
          "options": [
            {
              "key": "A",
              "text": "is a model of sustainability efforts",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P6-S2 明确阐明 LUMI 的标杆地位：'A great example of this is Europe's largest supercomputer, LUMI in Finland, which is astonishingly carbon negative... powered by river, heat warms town'（其绝佳典范便是芬兰超算 LUMI，实现了负碳排放，水力驱动，余热供暖）；P7-S1 进一步呼吁'deliver sustainable computing and deliver more LUMIs'（实现可持续计算并打造更多 LUMI）。A选项 is a model of sustainability efforts（是可持续发展努力的杰出典范/标杆）与 A great example... sustainable computing 完全同义对应。",
              "refSentences": [
                "P6-S2",
                "P7-S1"
              ]
            },
            {
              "key": "B",
              "text": "is a triumph against energy shortage",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 偷换概念】LUMI 解决的是算力的高能耗与碳排放污染问题，非战胜能源短缺危机。",
              "refSentences": [
                "P6-S2"
              ]
            },
            {
              "key": "C",
              "text": "owes much to global net-zero initiatives",
              "isCorrect": false,
              "distractorType": "因果倒置",
              "analysis": "【干扰特征: 因果颠倒】是全球净零目标需要依赖复制 LUMI 这样的可持续超算，非 LUMI 归功于全球倡议。",
              "refSentences": [
                "P7-S1"
              ]
            },
            {
              "key": "D",
              "text": "aims to explore the power of intelligence",
              "isCorrect": false,
              "distractorType": "字面望文生义",
              "analysis": "【干扰特征: 字面拼凑】power our intelligence 是 P6-S1 的哲学修辞，LUMI 的核心事实特征在于其绿色负碳排的运作模式。",
              "refSentences": [
                "P6-S1",
                "P6-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "LUMI is great example of carbon negative computing + deliver sustainable computing and more LUMIs = is a model of sustainability efforts。",
            "methodSummary": "标杆典范事实对应：great example of sustainable computing 转换为 model of sustainability efforts。"
          }
        },
        {
          "qIndex": 30,
          "type": "观点推断题",
          "tangchiModel": "唐迟观点题模型：通读全文作者的核心论点（P2: unarguable tools demand significant energy / P3: price we must pay for understanding world / cannot cut back research），锁定作者必然高度认同“高能耗的科研工作在探索认知世界中是不可避免的（Energy-intensive research work is inevitable）”",
          "stem": "Which of the following statements would the author agree with?",
          "stemKeywords": [
            "author agree with"
          ],
          "targetSentences": [
            "P1-S1",
            "P2-S1",
            "P3-S1",
            "P3-S2"
          ],
          "officialAnswer": "C",
          "options": [
            {
              "key": "A",
              "text": "Emission-free modelling demands extra funding.",
              "isCorrect": false,
              "distractorType": "无中生有资金",
              "analysis": "【干扰特征: 概念偏差】未提及零排放建模需要额外资金支持。",
              "refSentences": [
                "P1-S2",
                "P3-S2"
              ]
            },
            {
              "key": "B",
              "text": "The need for supercomputers is difficult to meet.",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 概念偏差】文章强调的是要用清洁能源实现超算的可持续化（sustainable computing），非超算需求难以满足。",
              "refSentences": [
                "P7-S1"
              ]
            },
            {
              "key": "C",
              "text": "Energy-intensive research work is inevitable.",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 全文核心论点高度概括】全文开篇作者严词驳斥削减高能耗科研建模的荒谬意图（P1），并在 P2-S1 明确指出科研工具消耗巨大能量是客观必然的（'It is unarguable that our laboratories, instruments, rockets... demand significant energy'——这是无可辩驳的必然事实）；并在 P3-S1 盖棺定论'this is a price we must pay for understanding the world'（这是我们认知世界必须付出的代价）。C选项 Energy-intensive research work is inevitable（高能耗的科学研究工作在客观上是不可避免的）是对作者贯穿全篇核心科学立场的精准提炼。",
              "refSentences": [
                "P1-S1",
                "P2-S1",
                "P3-S1",
                "P3-S2"
              ]
            },
            {
              "key": "D",
              "text": "The goals of researchers ought to be realistic.",
              "isCorrect": false,
              "distractorType": "空泛泛指",
              "analysis": "【干扰特征: 过于宽泛】文章鼓励科研人员放眼长远抱负拯救地球，非空泛劝诫务实。",
              "refSentences": [
                "P3-S2",
                "P7-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "instruments demand significant energy is unarguable + price we must pay to understand world = Energy-intensive research work is inevitable。",
            "methodSummary": "全文核心立场归纳：抓准 unarguable demand energy 与 price must pay 对应的 inevitable。"
          }
        }
      ]
    },
    {
      "id": "text3",
      "number": 3,
      "title": "The Streaming Wars and the Billion-Dollar Digital Piracy Menace",
      "chineseTitle": "好莱坞流媒体转型阵痛与盗版流媒体黑产冲击",
      "topic": "流媒体商业与数字反盗版 / 传统制片厂流媒体亏损、盗版流媒体黑产年入20亿、全美损失300亿与反盗版长期拉锯战",
      "overview": "本文深入揭露了全球流媒体时代下好莱坞传统巨头陷入巨额亏损与数字盗版黑产猖獗的残酷现实。正规流媒体平台（如 Disney+、Peacock）年亏数十亿美元，而零制作成本的非法盗播平台几秒内抓取片源，年狂揽20亿美元暴利，利润率逼近90%，使正规平台面临巨大的生存威胁。文章指出奈飞等合法平台早年曾协助遏制传统文件上传盗版，但如今新型盗版每年致使美国经济损失300亿美元并蒸发25万个就业岗位。电影协会（MPA）早先曾因硅谷不予配合导致立法流产，后成立跨国特遣队执法将北美盗版网站压缩至126家。分析师霍利警示未来损失将达1130亿美元，指出数字盗版在短期内绝不可能消退减少。",
      "paragraphs": [
        {
          "pIndex": 1,
          "mainIdea": "传统好莱坞在流媒体赛道面临年年亏损困境，而盗版流媒体黑产却靠盗播年入20亿美元暴利，使得正规流媒体面临严峻现实威胁",
          "logicRole": "提出正规流媒体巨额亏损与盗版流媒体黑产猖獗的严峻现实威胁 (Legitimate Streamers Face Real Threat)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P1-S1",
              "text": "Ever since taking on Netflix Inc. at its own game, old Hollywood has struggled to turn a profit in streaming, with the likes of Disney+, Peacock and Paramount+ losing billions of dollars each year, sparking concerns on Wall Street that the services will never be as profitable as cable once was.",
              "translation": "自从在流媒体赛道上与奈飞公司（Netflix）展开正面交锋以来，好莱坞传统电影制片厂巨头便一直在流媒体盈利泥潭中艰难苦苦挣扎——迪士尼+（Disney+）、孔雀（Peacock）以及派拉蒙+（Paramount+）等流媒体平台每年狂亏数十亿美元，引发了华尔街的深切忧虑：这些流媒体业务恐怕永远无法重现昔日有线电视时代的丰厚暴利。",
              "vocab": [
                {
                  "word": "since",
                  "ipa": "sɪns",
                  "meaning": "adv. 从那时以来 conj.",
                  "level": "green"
                },
                {
                  "word": "profit",
                  "ipa": "/ˈprɒfɪt/",
                  "meaning": "利润；获利（常用释义：收益；利益；使受益）",
                  "level": "red"
                },
                {
                  "word": "streaming",
                  "ipa": "ˈstriːmɪŋ",
                  "meaning": "n.串流，流式传播；（学生依能力分的）班/组 v.涌动；流出；",
                  "level": "green"
                },
                {
                  "word": "will",
                  "ipa": "/wɪl/",
                  "meaning": "遗嘱（常用释义：将要；意志；遗嘱）",
                  "level": "green"
                },
                {
                  "word": "profitable",
                  "ipa": "ˈprɑːfɪtəb(ə)l",
                  "meaning": "adj.盈利的，有利可图的；有益的",
                  "level": "green"
                },
                {
                  "word": "cable",
                  "ipa": "ˈkeɪb(ə)l",
                  "meaning": "n.电缆；钢缆，缆绳；电报；有线电视",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P1-S2",
              "text": "But the age of streaming has been a boon for some unintended winners: pirates that use software to rip a film or television show in seconds from legitimate online video platforms and host the titles on their own illegitimate services, which rake in about $2 billion annually from ads and subscriptions.",
              "translation": "然而，流媒体时代的全面降临却成为了一批意想不到的真正赢家的巨大横财摇钱树：数字盗版黑产犯罪分子利用破解软件在几秒钟内便能从正规在线视频平台抓取扒下整部电影或电视剧集，并将其架设在自己的非法盗播网站上——这些非法盗版平台每年单凭广告和会员订阅就能疯狂敛财吸金约20亿美元（legitimate streaming services are confronted with a real threat）！",
              "vocab": [
                {
                  "word": "streaming",
                  "ipa": "ˈstriːmɪŋ",
                  "meaning": "n.串流，流式传播；（学生依能力分的）班/组 v.涌动；流出；",
                  "level": "green"
                },
                {
                  "word": "boon",
                  "ipa": "buːn",
                  "meaning": "n.裨益，优势，有用的东西",
                  "level": "green"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "unintended",
                  "ipa": "ˌʌnɪnˈtendɪd",
                  "meaning": "adj.无意识的；非计划中的",
                  "level": "green"
                },
                {
                  "word": "use",
                  "ipa": "",
                  "meaning": "n./v. 使用，用途",
                  "level": "green"
                },
                {
                  "word": "software",
                  "ipa": "ˈsɔː ftwer",
                  "meaning": "n. 软件",
                  "level": "green"
                },
                {
                  "word": "rip",
                  "ipa": "rɪp",
                  "meaning": "v.（使）撕裂，（使）划破；迅速扯开 n.裂口；撕破",
                  "level": "green"
                },
                {
                  "word": "television",
                  "ipa": "",
                  "meaning": "n.电视，电视机；电视业",
                  "level": "red"
                },
                {
                  "word": "legitimate",
                  "ipa": "/lɪˈdʒɪtɪmət/",
                  "meaning": "合法且正当的（常用释义：合理的；正统的）",
                  "level": "red"
                },
                {
                  "word": "online",
                  "ipa": "/ˌɒnˈlaɪn/",
                  "meaning": "可通过互联网访问的；在线（常用释义：在线的；联网地）",
                  "level": "red"
                },
                {
                  "word": "video",
                  "ipa": "",
                  "meaning": "n.录像；录影；录像带；音乐录影；视频播放软件",
                  "level": "red"
                },
                {
                  "word": "about",
                  "ipa": "əˈbaʊt",
                  "meaning": "prep.从事于,忙于",
                  "level": "red"
                },
                {
                  "word": "annually",
                  "ipa": "ˈænjuəli",
                  "meaning": "adv.每年地",
                  "level": "red"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 2,
          "mainIdea": "零制作成本使非法流媒体网站利润率高达90%，美国电影协会正全力打击数千家非法平台",
          "logicRole": "揭示盗版网站暴利高达90%与好莱坞电影协会的打击行动 (90% Profit Margins of Pirates)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P2-S1",
              "text": "With no video production costs, illegal streaming sites have achieved profit margins approaching 90%, according to the Motion Picture Association (MPA), a trade group representing Hollywood studios that's working to crack down on the thousands of illegal platforms that have cropped up in recent years.",
              "translation": "代表好莱坞各大制片厂的行业组织“美国电影协会”（MPA）指出，由于无需承担任何高昂的影视制作成本，非法盗版流媒体网站的利润率竟然惊人地逼近90%；该协会目前正竭尽全力严厉打击近年来如雨后春笋般疯狂冒出的数千个非法盗播平台。",
              "vocab": [
                {
                  "word": "according to",
                  "ipa": "/əˈkɔːdɪŋ tuː/",
                  "meaning": "根据；按照……的说法（常用释义：用于交代观点来源）",
                  "level": "purple"
                },
                {
                  "word": "video",
                  "ipa": "",
                  "meaning": "n.录像；录影；录像带；音乐录影；视频播放软件",
                  "level": "red"
                },
                {
                  "word": "production",
                  "ipa": "prəˈdʌkʃn",
                  "meaning": "n. 生产；产量",
                  "level": "green"
                },
                {
                  "word": "illegal",
                  "ipa": "",
                  "meaning": "adj.不合法的",
                  "level": "green"
                },
                {
                  "word": "streaming",
                  "ipa": "ˈstriːmɪŋ",
                  "meaning": "n.串流，流式传播；（学生依能力分的）班/组 v.涌动；流出；",
                  "level": "green"
                },
                {
                  "word": "profit",
                  "ipa": "/ˈprɒfɪt/",
                  "meaning": "利润；获利（常用释义：收益；利益；使受益）",
                  "level": "red"
                },
                {
                  "word": "Motion",
                  "ipa": "ˈməʊʃn",
                  "meaning": "n.运动，移动；手势，动作；提议，议案 v.打手势，示意",
                  "level": "green"
                },
                {
                  "word": "Picture",
                  "ipa": "/ˈpɪktʃə/",
                  "meaning": "情况；局面（常用释义：图画；照片；整体形势）",
                  "level": "red"
                },
                {
                  "word": "Association",
                  "ipa": "əˌsəʊsiˈeɪʃn",
                  "meaning": "n. 协会；联合；联想",
                  "level": "green"
                },
                {
                  "word": "trade",
                  "ipa": "/treɪd/",
                  "meaning": "交换；用……换……（常用释义：贸易；交易；交换）",
                  "level": "red"
                },
                {
                  "word": "down",
                  "ipa": "daʊn",
                  "meaning": "v.使倒下,击倒",
                  "level": "red"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 3,
          "mainIdea": "奈飞等正规流媒体曾协助遏制传统文件上传盗版，但如今非法流媒体每年造成美国经济300亿美元收入损失与25万个就业岗位流失",
          "logicRole": "指出正规流媒体曾协助打击非法文件共享，阐述盗版对美国经济造成的巨大损害 (Economic Harm from Illegal Streaming)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P3-S1",
              "text": "Initially the rise of legitimate online businesses such as Netflix actually helped curb digital piracy, which had largely been based on file uploads.",
              "translation": "起初，诸如奈飞等正规合法在线流媒体商业平台的强势崛起，实际上曾发挥了积极作用，有力协助遏制打击了当年主要基于文件上传共享的传统数字盗版行径（streamers like Netflix played a part in the fight against illegal file-sharing）。",
              "vocab": [
                {
                  "word": "Initially",
                  "ipa": "/ɪˈnɪʃəli/",
                  "meaning": "起初；最初（常用释义：最初；开始时）",
                  "level": "red"
                },
                {
                  "word": "legitimate",
                  "ipa": "/lɪˈdʒɪtɪmət/",
                  "meaning": "合法且正当的（常用释义：合理的；正统的）",
                  "level": "red"
                },
                {
                  "word": "online",
                  "ipa": "/ˌɒnˈlaɪn/",
                  "meaning": "可通过互联网访问的；在线（常用释义：在线的；联网地）",
                  "level": "red"
                },
                {
                  "word": "curb",
                  "ipa": "/kɜːb/",
                  "meaning": "限制措施；遏制（常用释义：路缘；限制；控制）",
                  "level": "red"
                },
                {
                  "word": "digital",
                  "ipa": "ˈdɪdʒɪt(ə)l",
                  "meaning": "adj.数字的；数码的 n.数字电视",
                  "level": "green"
                },
                {
                  "word": "piracy",
                  "ipa": "ˈpaɪrəsi",
                  "meaning": "n.海盗行为；盗版行为",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            },
            {
              "sIndex": 2,
              "id": "P3-S2",
              "text": "But now piracy involving illegal streaming services as well as file-sharing costs the US economy about $30 billion in lost revenue a year and some 250,000 jobs, estimates the US Chamber of Commerce's Global Innovation Policy Center. The global impact is about $71 billion annually.",
              "translation": "但据美国商会全球创新政策中心权威估算，如今涵盖非法流媒体盗播以及文件共享在内的新型数字盗版，每年给美国经济造成了高达约300亿美元的巨额收入损失，并直接导致约25万个就业岗位彻底蒸发消失（emphasized economic harm from illegal streaming）；其在全球范围内造成的年度经济破坏更是高达约710亿美元之巨！",
              "vocab": [
                {
                  "word": "piracy",
                  "ipa": "ˈpaɪrəsi",
                  "meaning": "n.海盗行为；盗版行为",
                  "level": "green"
                },
                {
                  "word": "illegal",
                  "ipa": "",
                  "meaning": "adj.不合法的",
                  "level": "green"
                },
                {
                  "word": "streaming",
                  "ipa": "ˈstriːmɪŋ",
                  "meaning": "n.串流，流式传播；（学生依能力分的）班/组 v.涌动；流出；",
                  "level": "green"
                },
                {
                  "word": "file-sharing",
                  "ipa": "ˈfaɪlʃeərɪŋ",
                  "meaning": "n.文件共享",
                  "level": "green"
                },
                {
                  "word": "about",
                  "ipa": "əˈbaʊt",
                  "meaning": "prep.从事于,忙于",
                  "level": "red"
                },
                {
                  "word": "revenue",
                  "ipa": "ˈrevənuː",
                  "meaning": "n.税收收入；收益",
                  "level": "green"
                },
                {
                  "word": "Chamber",
                  "ipa": "ˈtʃeɪmbə(r)",
                  "meaning": "n.房间，室；会议厅，会所；议院；腔，膛；洞穴",
                  "level": "green"
                },
                {
                  "word": "Commerce",
                  "ipa": "ˈkɒmɜːs",
                  "meaning": "n.贸易，商业",
                  "level": "green"
                },
                {
                  "word": "Innovation",
                  "ipa": "/ˌɪnəˈveɪʃən/",
                  "meaning": "创新；新思想（常用释义：革新；创新成果）",
                  "level": "green"
                },
                {
                  "word": "Policy",
                  "ipa": "ˈpɔlɪsɪ",
                  "meaning": "n. 政策，方针，原则",
                  "level": "green"
                },
                {
                  "word": "impact",
                  "ipa": "",
                  "meaning": "n.撞击，冲击力；巨大影响，强大作用",
                  "level": "red"
                },
                {
                  "word": "annually",
                  "ipa": "ˈænjuəli",
                  "meaning": "adv.每年地",
                  "level": "red"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 4,
          "mainIdea": "电影协会 CEO 里夫金指出盗版是跨国有组织犯罪，曾因好莱坞与硅谷未能达成共识导致国会反盗版立法流产",
          "logicRole": "指出 MPA 曾因硅谷未予配合而在国会立法上受挫，后成立 ACE 执法特遣队 (MPA Formed ACE Task Force)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P4-S1",
              "text": "\"The people who are stealing our movies and our television shows and operating piracy sites are not mom and pop operations,\" says Charlie Rivkin, chief executive officer of the MPA. \"This is organized crime.\"",
              "translation": "“那些大肆偷窃我们电影和电视剧集、运营盗版网站的幕后黑手，绝非什么小打小闹的家庭作坊式经营，”美国电影协会首席执行官查理·里夫金严正指出，“这完全是彻头彻尾的跨国有组织犯罪黑产！”",
              "vocab": [
                {
                  "word": "television",
                  "ipa": "",
                  "meaning": "n.电视，电视机；电视业",
                  "level": "red"
                },
                {
                  "word": "piracy",
                  "ipa": "ˈpaɪrəsi",
                  "meaning": "n.海盗行为；盗版行为",
                  "level": "green"
                },
                {
                  "word": "executive",
                  "ipa": "/ɪɡˈzekjətɪv/",
                  "meaning": "行政的、行政部门的（常用释义：高级管理人员（n.））",
                  "level": "red"
                },
                {
                  "word": "officer",
                  "ipa": "ˈɔːfɪsər",
                  "meaning": "n. 军官;公务员,官员；",
                  "level": "green"
                },
                {
                  "word": "organized",
                  "ipa": "/ˈɔːɡənaɪzd/",
                  "meaning": "有组织的；有条理的（常用释义：安排良好的）",
                  "level": "green"
                },
                {
                  "word": "crime",
                  "ipa": "kraɪm",
                  "meaning": "n. （法律上的）罪，犯",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P4-S2",
              "text": "Rivkin joined the MPA in 2017 after the organization failed five years earlier to build consensus between Hollywood and Silicon Valley to win passage of legislation in Congress aimed at stopping online piracy.",
              "translation": "里夫金于2017年出掌美国电影协会；此前五年，该组织曾因未能成功在好莱坞与硅谷科技巨头之间建立起立法共识（failed to build consensus between Hollywood and Silicon Valley / was denied cooperation by Silicon Valley），导致旨在遏制网络盗版的国会关键法案最终惨遭流产搁浅。",
              "vocab": [
                {
                  "word": "consensus",
                  "ipa": "kənˈsensəs",
                  "meaning": "n.共识；一致意见",
                  "level": "green"
                },
                {
                  "word": "Valley",
                  "ipa": "ˈvælɪ",
                  "meaning": "n. 山谷, 溪谷",
                  "level": "green"
                },
                {
                  "word": "passage",
                  "ipa": "ˈpæsɪdʒ",
                  "meaning": "n.（文章等的）一节，",
                  "level": "green"
                },
                {
                  "word": "legislation",
                  "ipa": "ˌledʒɪsˈleɪʃ(ə)n",
                  "meaning": "n.立法；法规",
                  "level": "green"
                },
                {
                  "word": "Congress",
                  "ipa": "",
                  "meaning": "n.国民大会，代表大会，（美国及其他国家的）国会，议会",
                  "level": "red"
                },
                {
                  "word": "online",
                  "ipa": "/ˌɒnˈlaɪn/",
                  "meaning": "可通过互联网访问的；在线（常用释义：在线的；联网地）",
                  "level": "red"
                },
                {
                  "word": "piracy",
                  "ipa": "ˈpaɪrəsi",
                  "meaning": "n.海盗行为；盗版行为",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            },
            {
              "sIndex": 3,
              "id": "P4-S3",
              "text": "In 2017 the association formed the Alliance for Creativity and Entertainment (ACE), an enforcement task force of about 100 detectives circling the globe to help local authorities arrest streaming pirates.",
              "translation": "2017年，该协会正式牵头组建了“创意与娱乐联盟”（ACE）——这是一支由约100名资深侦探组成的跨国执法特遣队，常年穿梭奔波于全球各地，协助多国地方执法当局全力缉捕流媒体盗版主犯。",
              "vocab": [
                {
                  "word": "association",
                  "ipa": "əˌsəʊsiˈeɪʃn",
                  "meaning": "n. 协会；联合；联想",
                  "level": "green"
                },
                {
                  "word": "Alliance",
                  "ipa": "əˈlaɪəns",
                  "meaning": "n.联盟；结盟",
                  "level": "green"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "Entertainment",
                  "ipa": "entəˈteɪnmənt",
                  "meaning": "n. 娱乐",
                  "level": "green"
                },
                {
                  "word": "enforcement",
                  "ipa": "/ɪnˈfɔːsmənt/",
                  "meaning": "执法；执行、实施（常用释义：强制执行）",
                  "level": "red"
                },
                {
                  "word": "about",
                  "ipa": "əˈbaʊt",
                  "meaning": "prep.从事于,忙于",
                  "level": "red"
                },
                {
                  "word": "arrest",
                  "ipa": "əˈrest",
                  "meaning": "v.逮捕，拘留；抑制；吸引；心跳停止 n.逮捕；终止",
                  "level": "green"
                },
                {
                  "word": "streaming",
                  "ipa": "ˈstriːmɪŋ",
                  "meaning": "n.串流，流式传播；（学生依能力分的）班/组 v.涌动；流出；",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 5,
          "mainIdea": "ACE 执法行动与2020年联邦重罪立法促使北美非法流媒体平台大幅缩减至126家",
          "logicRole": "介绍联合执法与联邦刑法立法将非法平台压缩至126家 (ACE Reduced Piracy Sites)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P5-S1",
              "text": "ACE says it's helped shrink the number of illegal streaming services in North America to 126, from more than 1,400 in 2018, aided in part by the MPA's support for a 2020 federal law that made large-scale streaming of copyrighted material a serious crime.",
              "translation": "ACE 宣布，他们已成功协助将北美的非法流媒体盗播网站数量从2018年的1400多家大幅压缩削减至目前的126家；这一战果部分得益于美国电影协会所鼎力支持的2020年联邦法律——该法已正式将大规模盗播受版权保护影视作品的行为定性为严重的联邦重罪。",
              "vocab": [
                {
                  "word": "in part",
                  "ipa": "/ɪn pɑːt/",
                  "meaning": "部分地；在一定程度上（常用释义：partly；to some extent）",
                  "level": "red"
                },
                {
                  "word": "shrink",
                  "ipa": "ʃrɪŋk",
                  "meaning": "v.（使）缩小；缩水；退缩；畏避 n.精神病学家，心理学家",
                  "level": "green"
                },
                {
                  "word": "illegal",
                  "ipa": "",
                  "meaning": "adj.不合法的",
                  "level": "green"
                },
                {
                  "word": "streaming",
                  "ipa": "ˈstriːmɪŋ",
                  "meaning": "n.串流，流式传播；（学生依能力分的）班/组 v.涌动；流出；",
                  "level": "green"
                },
                {
                  "word": "support",
                  "ipa": "/səˈpɔːt/",
                  "meaning": "资助、维持或运营知识库（常用释义：支持观点；支撑重量；支持物）",
                  "level": "red"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "federal",
                  "ipa": "ˈfɛdərəl",
                  "meaning": "adj. 联邦的，联盟的",
                  "level": "green"
                },
                {
                  "word": "large-scale",
                  "ipa": "ˌlɑːdʒˈskeɪl",
                  "meaning": "adj.大规模的，大范围的",
                  "level": "green"
                },
                {
                  "word": "copyrighted",
                  "ipa": "'kɔpiˌraitid",
                  "meaning": "adj.有版权的",
                  "level": "green"
                },
                {
                  "word": "material",
                  "ipa": "məˈtɪriəl",
                  "meaning": "n.衣料；材料；素材 adj.物质的；重要的",
                  "level": "green"
                },
                {
                  "word": "crime",
                  "ipa": "kraɪm",
                  "meaning": "n. （法律上的）罪，犯",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 6,
          "mainIdea": "咨询机构预测正规流媒体盗版损失将达1130亿美元，分析师霍利指出数字盗版短期内绝不可能消退减少",
          "logicRole": "分析师指出盗版未来两年仍将造成千亿损失且短期内难以减少 (Hawley: Piracy Unlikely to Diminish in Near Future)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P6-S1",
              "text": "Consulting firm Parks Associates predicts that legitimate US streaming services' cumulative loss from piracy since 2022 will reach $113 billion in the next two years.",
              "translation": "权威咨询机构 Parks Associates 警示预测：自2022年以来，美国正规合法流媒体平台因数字盗版所遭受的累计经济损失，在未来两年内将触目惊心地达到整整1130亿美元之巨！",
              "vocab": [
                {
                  "word": "legitimate",
                  "ipa": "/lɪˈdʒɪtɪmət/",
                  "meaning": "合法且正当的（常用释义：合理的；正统的）",
                  "level": "red"
                },
                {
                  "word": "streaming",
                  "ipa": "ˈstriːmɪŋ",
                  "meaning": "n.串流，流式传播；（学生依能力分的）班/组 v.涌动；流出；",
                  "level": "green"
                },
                {
                  "word": "cumulative",
                  "ipa": "ˈkjuːmjələtɪv",
                  "meaning": "adj.积累的，渐增的；累计的，累积的",
                  "level": "green"
                },
                {
                  "word": "piracy",
                  "ipa": "ˈpaɪrəsi",
                  "meaning": "n.海盗行为；盗版行为",
                  "level": "green"
                },
                {
                  "word": "since",
                  "ipa": "sɪns",
                  "meaning": "adv. 从那时以来 conj.",
                  "level": "green"
                },
                {
                  "word": "will",
                  "ipa": "/wɪl/",
                  "meaning": "遗嘱（常用释义：将要；意志；遗嘱）",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P6-S2",
              "text": "\"While there is some optimism that emerging countermeasures and best practices may see piracy begin to plateau by 2027, there is no consensus among stakeholders as to when it may begin to decline,\" says analyst Steve Hawley.",
              "translation": "“尽管业界存在一定的乐观预期，认为新兴的反制手段与行业最佳实践或许能让盗版蔓延态势在2027年左右迎来平台期；但在盗版何时才会真正开始出现拐点下降消退这一核心问题上，各方利益相关者之间根本尚未达成任何共识（digital piracy is unlikely to diminish in the near future），”行业资深分析师史蒂夫·霍利指出。",
              "vocab": [
                {
                  "word": "optimism",
                  "ipa": "",
                  "meaning": "n.乐观；乐观主义",
                  "level": "green"
                },
                {
                  "word": "piracy",
                  "ipa": "ˈpaɪrəsi",
                  "meaning": "n.海盗行为；盗版行为",
                  "level": "green"
                },
                {
                  "word": "plateau",
                  "ipa": "ˈplætəʊ",
                  "meaning": "n.高原；稳定期，停滞期 v.趋于平稳，进入停滞期（~out）",
                  "level": "green"
                },
                {
                  "word": "consensus",
                  "ipa": "kənˈsensəs",
                  "meaning": "n.共识；一致意见",
                  "level": "green"
                },
                {
                  "word": "decline",
                  "ipa": "/dɪˈklaɪn/",
                  "meaning": "拒绝、不接受论文（常用释义：下降；衰退；婉言拒绝）",
                  "level": "red"
                },
                {
                  "word": "analyst",
                  "ipa": "ˈænəlɪst",
                  "meaning": "n.分析者；专家",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        }
      ],
      "questions": [
        {
          "qIndex": 31,
          "type": "细节事实题",
          "tangchiModel": "唐迟细节题模型：抓住第一段正规合法流媒体平台（如 Disney+、Peacock）面临的残酷现实（struggled to turn profit, losing billions each year while pirates rip films in seconds and rake in $2 billion），锁定正规流媒体平台正面临着来自盗版黑产的严峻现实威胁（confronted with a real threat）",
          "stem": "According to Paragraph 1, legitimate streaming services",
          "stemKeywords": [
            "Paragraph 1",
            "legitimate streaming services"
          ],
          "targetSentences": [
            "P1-S1",
            "P1-S2"
          ],
          "officialAnswer": "D",
          "options": [
            {
              "key": "A",
              "text": "have drawn lessons from Hollywood",
              "isCorrect": false,
              "distractorType": "偷换概念主体",
              "analysis": "【干扰特征: 概念混淆】这些平台本身就是老牌好莱坞制片厂亲手打造的（old Hollywood entered streaming），非向好莱坞吸取教训。",
              "refSentences": [
                "P1-S1"
              ]
            },
            {
              "key": "B",
              "text": "have surpassed cable in revenue",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实完全相反】华尔街深感忧虑其盈利能力永远无法企及昔日的有线电视（never be as profitable as cable）。",
              "refSentences": [
                "P1-S1"
              ]
            },
            {
              "key": "C",
              "text": "are unpopular with advertisers",
              "isCorrect": false,
              "distractorType": "张冠李戴盗版网站",
              "analysis": "【干扰特征: 偷换对象】从广告和会员中狂敛20亿暴利的是非法盗版网站（illegitimate services rake in from ads），非正规平台不受欢迎。",
              "refSentences": [
                "P1-S2"
              ]
            },
            {
              "key": "D",
              "text": "are confronted with a real threat",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P1-S1 和 P1-S2 明确展现正规流媒体平台的巨大生存危机：正规平台巨亏数十亿，而盗版平台几秒钟内盗取其片源并掠夺性狂揽20亿美元。正规合法流媒体平台显然正面临着来自非法盗版黑产的巨大现实威胁与生存挤压（are confronted with a real threat）。D选项完全精准概括了第一段的核心处境。",
              "refSentences": [
                "P1-S1",
                "P1-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "legitimate streamers lose billions while pirates rip shows in seconds and rake in $2 billion = legitimate streaming services are confronted with a real threat。",
            "methodSummary": "首段危机处境概括：正规平台亏损与盗版掠夺并存，直接锁定 confronted with a real threat。"
          }
        },
        {
          "qIndex": 32,
          "type": "细节事实题",
          "tangchiModel": "唐迟细节题模型：抓住第三段首句关于 Netflix 等正规流媒体平台在历史上发挥的作用（Initially the rise of legitimate online businesses such as Netflix actually helped curb digital piracy, which had largely been based on file uploads），锁定奈飞等流媒体曾参与并助力遏制了非法文件共享盗版（played a part in the fight against illegal file-sharing）",
          "stem": "It can be learned that streamers like Netflix",
          "stemKeywords": [
            "streamers like Netflix"
          ],
          "targetSentences": [
            "P3-S1"
          ],
          "officialAnswer": "A",
          "options": [
            {
              "key": "A",
              "text": "played a part in the fight against illegal file-sharing",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 原文同义复现】P3-S1 明确交代奈飞等流媒体的历史功绩：'Initially the rise of legitimate online businesses such as Netflix actually helped curb digital piracy, which had largely been based on file uploads'（起初，像奈飞这样的正规合法在线业务的崛起，实际上曾帮助遏制了当时主要基于文件上传的数字盗版）。curb digital piracy based on file uploads 与 played a part in the fight against illegal file-sharing（在打击非法文件共享盗版的斗争中发挥了一定作用）完全同义对应。A选项精准吻合。",
              "refSentences": [
                "P3-S1"
              ]
            },
            {
              "key": "B",
              "text": "reaped benefits from the war with digital pirates",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实完全相反】正规平台目前因盗版每年蒙受千亿美元计的惨重经济损失（loss of revenue），绝非从中获利（reaped benefits）。",
              "refSentences": [
                "P3-S2",
                "P6-S1"
              ]
            },
            {
              "key": "C",
              "text": "promised to become big job creators in the US",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 概念偏差】第三段讲的是盗版导致美国损失了25万个就业岗位（lost 250,000 jobs），非奈飞承诺创造就业。",
              "refSentences": [
                "P3-S2"
              ]
            },
            {
              "key": "D",
              "text": "used to collaborate with file-uploading platforms",
              "isCorrect": false,
              "distractorType": "正反颠倒对抗关系",
              "analysis": "【干扰特征: 敌我颠倒】奈飞等合法平台是在遏制打击（curb）基于文件上传的盗版，绝非与文件上传盗版平台合作（collaborate）。",
              "refSentences": [
                "P3-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "Netflix helped curb digital piracy largely based on file uploads = played a part in the fight against illegal file-sharing。",
            "methodSummary": "原句细节直接对应：helped curb piracy based on file uploads 转换为 played a part in fight against illegal file-sharing。"
          }
        },
        {
          "qIndex": 33,
          "type": "细节推断题",
          "tangchiModel": "唐迟推断题模型：抓住第四段关于美国电影协会（MPA）历史挫折的陈述（failed five years earlier to build consensus between Hollywood and Silicon Valley to win passage of legislation），锁定可以合理推断出 MPA 曾遭到硅谷科技巨头的拒绝合作与不予配合（was denied cooperation by Silicon Valley）",
          "stem": "It can be inferred from Paragraph 4 that the MPA",
          "stemKeywords": [
            "inferred from Paragraph 4",
            "the MPA"
          ],
          "targetSentences": [
            "P4-S2"
          ],
          "officialAnswer": "A",
          "options": [
            {
              "key": "A",
              "text": "was denied cooperation by Silicon Valley",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 合理推断】P4-S2 明确交代 MPA 早先立法流产的历史原因：'the organization failed five years earlier to build consensus between Hollywood and Silicon Valley to win passage of legislation in Congress aimed at stopping online piracy'（MPA 五年前曾因未能成功在好莱坞与硅谷科技巨头之间建立起共识，导致反盗版国会法案流产）。好莱坞制片厂协会（MPA）未能与硅谷达成共识，其本质正是遭到了硅谷科技巨头的反对、抵触与拒绝合作（was denied cooperation by Silicon Valley）。A选项完全精准契合推理逻辑。",
              "refSentences": [
                "P4-S2"
              ]
            },
            {
              "key": "B",
              "text": "led a national protest against online piracy",
              "isCorrect": false,
              "distractorType": "偷换维权手段",
              "analysis": "【干扰特征: 偷换手段】MPA 采取的是推动国会立法和组建特遣队执法缉捕，非组织领导全国性民众抗议示威（national protest）。",
              "refSentences": [
                "P4-S2",
                "P4-S3"
              ]
            },
            {
              "key": "C",
              "text": "was urged to form an enforcement task force",
              "isCorrect": false,
              "distractorType": "主动被动颠倒",
              "analysis": "【干扰特征: 主被动颠倒】MPA 是主动牵头组建了 ACE 执法特遣队（formed the Alliance），非被外部力量催促强迫组建。",
              "refSentences": [
                "P4-S3"
              ]
            },
            {
              "key": "D",
              "text": "failed to win support from local authorities",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实相反】P4-S3 明确指出 ACE 正在全球各地密切协助地方执法当局（help local authorities arrest pirates），合作良好。",
              "refSentences": [
                "P4-S3"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "failed to build consensus between Hollywood (MPA) and Silicon Valley for legislation = was denied cooperation by Silicon Valley。",
            "methodSummary": "因果推断法：未能达成立法共识即意味着遭到了利益对手硅谷的拒绝配合（denied cooperation）。"
          }
        },
        {
          "qIndex": 34,
          "type": "观点细节题",
          "tangchiModel": "唐迟观点题模型：抓住第六段行业分析师 Steve Hawley 的核心论断（there is no consensus among stakeholders as to when piracy may begin to decline / plateau by 2027），锁定数字盗版在短期或可预见的未来内绝不可能消退减少（unlikely to diminish in the near future）",
          "stem": "According to Hawley, digital piracy",
          "stemKeywords": [
            "According to Hawley",
            "digital piracy"
          ],
          "targetSentences": [
            "P6-S1",
            "P6-S2"
          ],
          "officialAnswer": "C",
          "options": [
            {
              "key": "A",
              "text": "cannot be checked in spite of new legislation",
              "isCorrect": false,
              "distractorType": "过于绝对",
              "analysis": "【干扰特征: 过于绝望绝对】霍利认为到2027年可能迎来平台期，新法律也成功将北美网站压缩至126家，非完全不可遏制。",
              "refSentences": [
                "P5-S1",
                "P6-S2"
              ]
            },
            {
              "key": "B",
              "text": "will possibly overwhelm legitimate streamers",
              "isCorrect": false,
              "distractorType": "过度推断夸大",
              "analysis": "【干扰特征: 夸大后果】虽然造成千亿损失，但霍利并未断言盗版会彻底压垮压倒正规流媒体巨头。",
              "refSentences": [
                "P6-S1",
                "P6-S2"
              ]
            },
            {
              "key": "C",
              "text": "is unlikely to diminish in the near future",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P6-S2 明确引述霍利的原话：'there is no consensus among stakeholders as to when it may begin to decline'（各方利益相关者对盗版何时才会开始真正下降消退根本毫无共识，即使乐观估计也得等到2027年才可能进入平台期）。这一清醒判断表明：数字盗版在可预见的近期内绝不可能消退减少（is unlikely to diminish in the near future）。C选项精准契合分析师观点。",
              "refSentences": [
                "P6-S2"
              ]
            },
            {
              "key": "D",
              "text": "has been underestimated by some analysts",
              "isCorrect": false,
              "distractorType": "无中生有",
              "analysis": "【干扰特征: 概念偏差】未提及盗版危害被其他分析师低估。",
              "refSentences": [
                "P6-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "no consensus as to when it may begin to decline (plateau only by 2027) = is unlikely to diminish in the near future。",
            "methodSummary": "时间预期推断法：何时 decline 毫无共识且2027年前仅能 plateau，推导出 unlikely to diminish in the near future。"
          }
        },
        {
          "qIndex": 35,
          "type": "主旨强调题",
          "tangchiModel": "唐迟主旨题模型：通读全文，提炼全篇从好莱坞流媒体亏损数十亿、盗版平台敛财20亿、美国经济损失300亿与25万就业、未来累计损失1130亿等数据链条中反复强调的核心——非法流媒体盗版造成的巨大经济破坏与损害（The economic harm from illegal streaming）",
          "stem": "Which of the following is emphasized in the text?",
          "stemKeywords": [
            "emphasized in the text"
          ],
          "targetSentences": [
            "P1-S2",
            "P2-S1",
            "P3-S2",
            "P6-S1"
          ],
          "officialAnswer": "D",
          "options": [
            {
              "key": "A",
              "text": "The need to coordinate anti-piracy action.",
              "isCorrect": false,
              "distractorType": "以偏概全局部对策",
              "analysis": "【干扰特征: 局部手段】执法协作仅在第4-5段提及，全文贯穿始终的震撼主线是盗版造成的惊人经济破坏。",
              "refSentences": [
                "P4-S3",
                "P5-S1"
              ]
            },
            {
              "key": "B",
              "text": "The criminal nature of copyright violation.",
              "isCorrect": false,
              "distractorType": "局部定性",
              "analysis": "【干扰特征: 局部定性】定性为有组织犯罪只是第4段里夫金的控诉，未能统领全文的数据损失主轴。",
              "refSentences": [
                "P4-S1"
              ]
            },
            {
              "key": "C",
              "text": "The prospect of eliminating online piracy.",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实相反】文末明确指出短期内盗版难以消退，消灭盗版前景依然极其严峻遥远。",
              "refSentences": [
                "P6-S2"
              ]
            },
            {
              "key": "D",
              "text": "The economic harm from illegal streaming.",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 全文核心强调要点】纵观全文，作者通过极其扎实触目惊心的经济数据链条贯穿始终反复强调：第1段指出盗版年敛20亿美元暴利；第2段指出其利润率高达90%；第3段指出每年致使美国经济损失300亿美元并丧失25万就业岗位、全球损失710亿美元；第6段指出未来两年正规平台累计损失将达1130亿美元。D选项 The economic harm from illegal streaming（非法流媒体盗播所造成的巨大经济危害与损失）全面、精准且有力地概括了全文反复着重强调的核心议题。",
              "refSentences": [
                "P1-S2",
                "P2-S1",
                "P3-S2",
                "P6-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "$2 billion raked in + $30 billion lost and 250k jobs + $113 billion cumulative loss = The economic harm from illegal streaming。",
            "methodSummary": "核心数据链条归纳法：通篇围绕千亿级经济损失数据展开，直接锁定 The economic harm from illegal streaming。"
          }
        }
      ]
    },
    {
      "id": "text4",
      "number": 4,
      "title": "The Digital Dark Age: Preserving Intangible Historical Records",
      "chineseTitle": "数字时代历史文献的保存危机与图书馆员的坚守",
      "topic": "图书馆学与数字资产长期留存 / 实体文物与无形数字信息的生存对比、数字格式淘汰、历史记录断层危机与图书馆员的不可或缺",
      "overview": "本文探讨了在数字化席卷一切的现代社会中，无形数字历史文献所面临的严峻保存危机。与古董店里历经数百年长存的实体文物（本身即是信息载体、直接传达信息且无需刻意保存）形成鲜明反差的是，原生数字信息若缺乏维护极易湮灭。图书馆员的无私奉献让手机随时随地可查阅资料，却导致用户产生图书馆不再重要的严重错觉，贬低了图书馆的崇高价值。文章以过时的 ZIP 软盘生动展现了格式淘汰导致内部数据彻底灭失的巨大风险与历史记录断代的深夜噩梦。文章最后总结指出，保存数字历史记录充满重重艰巨挑战，但图书馆员的默默坚守确保了人类数字记忆绝不会走向彻底湮灭。",
      "paragraphs": [
        {
          "pIndex": 1,
          "mainIdea": "古董店里的照片信件等历史遗物虽看似短暂却能历经数十年数百年长存，引出历史遗物的长期保存议题",
          "logicRole": "以古董店实体文物长存为由头引出无形数字时代的文献保存危机 (Bring up Preservation Issue)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P1-S1",
              "text": "Visit any antiques store and you'll encounter artifacts from the past: photographs, letters, a brochure detailing the Sinclair dinosaur exhibit from the 1964-1965 World's Fair, the ephemera of history.",
              "translation": "走进任何一家古董旧货店，你都会邂逅无数来自过往岁月的珍贵历史器物遗存：泛黄的老照片、手写书信、详细介绍1964至1965年世博会辛克莱恐龙展览的折页宣传册——这些看似过眼云烟的“历史短暂遗存物”（ephemera of history）。",
              "vocab": [
                {
                  "word": "encounter",
                  "ipa": "ɪnˈkaʊntə(r)",
                  "meaning": "n./v. 遭遇；遇到",
                  "level": "green"
                },
                {
                  "word": "brochure",
                  "ipa": "ˈbrəʊʃə(r)",
                  "meaning": "n.小册子，资料（或广告）手册",
                  "level": "green"
                },
                {
                  "word": "exhibit",
                  "ipa": "/ɪɡˈzɪbɪt/",
                  "meaning": "表现出，显示出（常用释义：显示；展览；展品）",
                  "level": "red"
                },
                {
                  "word": "Fair",
                  "ipa": "feə(r)",
                  "meaning": "adj. 公平的，合理的 adj.",
                  "level": "green"
                },
                {
                  "word": "ephemera",
                  "ipa": "ɪˈfemərə",
                  "meaning": "n.蜉蝣",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P1-S2",
              "text": "Yet these objects aren't truly ephemeral, because they are still here, decades, even centuries later. Why? Because they're tangible.",
              "translation": "然而，这些物件却并非真正如浮云般转瞬即逝，因为在历经数十年、乃至数个世纪的风霜洗礼之后，它们依然完好地留存于世。为什么？因为它们是实实在在、触手可及的“有形实体”（tangible / bring up the issue of preservation）！",
              "vocab": [
                {
                  "word": "Yet",
                  "ipa": "/jet/",
                  "meaning": "然而；但是（常用释义：然而；还；尚）",
                  "level": "red"
                },
                {
                  "word": "ephemeral",
                  "ipa": "ɪˈfemərəl",
                  "meaning": "adj.短暂的；（主指植物）短生的，短命的",
                  "level": "green"
                },
                {
                  "word": "still",
                  "ipa": "/stɪl/",
                  "meaning": "尽管如此；然而（常用释义：仍然；但是，尽管如此）",
                  "level": "red"
                },
                {
                  "word": "tangible",
                  "ipa": "ˈtændʒəb(ə)l",
                  "meaning": "adj.明确的，真实的；可触摸的，可感知的",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 2,
          "mainIdea": "实体器物本身即是信息载体能更直接传达信息，无需刻意保护即可自然幸存；图书馆员将实体数字化带来了浩瀚历史财富",
          "logicRole": "对比实体文物直接传达信息与图书馆员数字化带来的浩瀚财富 (Tangible Convey Information Directly)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P2-S1",
              "text": "Have you pondered the life cycle of intangible formats, digital information, given that those who produce these artifacts seldom make provision for their long-term preservation?",
              "translation": "你是否曾深入思考过无形格式——即纯粹的数字信息——的生命周期？尤其是考虑到那些生产制造此类数字制品的创作者们，在创作时几乎极少会为其日后的长期留存保存做出任何周全预案？",
              "vocab": [
                {
                  "word": "intangible",
                  "ipa": "ɪnˈtændʒəb(ə)l",
                  "meaning": "adj.不可捉摸的，难以确定的；（资产/利益）无形的",
                  "level": "green"
                },
                {
                  "word": "digital",
                  "ipa": "ˈdɪdʒɪt(ə)l",
                  "meaning": "adj.数字的；数码的 n.数字电视",
                  "level": "green"
                },
                {
                  "word": "information",
                  "ipa": "ɪnfəˈmeɪʃ(ə)n",
                  "meaning": "n. 信息",
                  "level": "green"
                },
                {
                  "word": "produce",
                  "ipa": "",
                  "meaning": "v.生产",
                  "level": "green"
                },
                {
                  "word": "seldom",
                  "ipa": "ˈseldəm",
                  "meaning": "adv.不常，很少",
                  "level": "green"
                },
                {
                  "word": "provision",
                  "ipa": "/prəˈvɪʒən/",
                  "meaning": "法律条款、规定（常用释义：提供；供应；预备）",
                  "level": "red"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "preservation",
                  "ipa": "ˌprezərˈveɪʃ(ə)n",
                  "meaning": "n.保存；保护；维持",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P2-S2",
              "text": "For millennia, we've known what we've known due to artifacts that have survived, often despite their original creators' neglect. The thing itself is the medium that delivers the information.",
              "translation": "数千年来，人类之所以能够通晓过往的历史认知，全赖那些历经劫难而幸存下来的古代文物——哪怕其最初的原作者当年对其疏于照料。实体器物本身就是直接承载并传递历史信息的物理介质（tangible artifacts convey information in a more direct way / thing itself is the medium）！",
              "vocab": [
                {
                  "word": "For",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "millennia",
                  "ipa": "mɪˈleniə",
                  "meaning": "n.千年期；千周年纪念日（millennium 的复数）",
                  "level": "green"
                },
                {
                  "word": "due",
                  "ipa": "/djuː/",
                  "meaning": "应有的；适当的（常用释义：到期的；预定的；由于）",
                  "level": "red"
                },
                {
                  "word": "despite",
                  "ipa": "/dɪˈspaɪt/",
                  "meaning": "尽管（常用释义：即使；不管）",
                  "level": "red"
                },
                {
                  "word": "original",
                  "ipa": "/əˈrɪdʒənəl/",
                  "meaning": "最初的；早期的（常用释义：原创的；原来的；原作）",
                  "level": "red"
                },
                {
                  "word": "neglect",
                  "ipa": "/nɪˈɡlekt/",
                  "meaning": "忽视；未给予应有重视（常用释义：疏忽；疏于照料）",
                  "level": "red"
                },
                {
                  "word": "medium",
                  "ipa": "ˈmiːdɪəm",
                  "meaning": "n.媒体,中间的,中等",
                  "level": "green"
                },
                {
                  "word": "information",
                  "ipa": "ɪnfəˈmeɪʃ(ə)n",
                  "meaning": "n. 信息",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            },
            {
              "sIndex": 3,
              "id": "P2-S3",
              "text": "At the time of creation, no attempts were made at intentional preservation, yet analog materials have a chance of surviving and serving as the historical record that biographers, historians, and novelists rely on.",
              "translation": "在被创造之初，人们往往并未做出任何刻意保存的努力，但传统的模拟实体材料却天然拥有一线幸存生机，得以代代相传成为传记作家、历史学家和小说家所赖以考据的珍贵历史记录。",
              "vocab": [
                {
                  "word": "rely on",
                  "ipa": "/rɪˈlaɪ ɒn/",
                  "meaning": "依赖；依靠（常用释义：depend on）",
                  "level": "purple"
                },
                {
                  "word": "intentional",
                  "ipa": "ɪnˈtenʃən(ə)l",
                  "meaning": "adj.故意的，蓄意的",
                  "level": "green"
                },
                {
                  "word": "preservation",
                  "ipa": "ˌprezərˈveɪʃ(ə)n",
                  "meaning": "n.保存；保护；维持",
                  "level": "green"
                },
                {
                  "word": "yet",
                  "ipa": "/jet/",
                  "meaning": "然而；但是（常用释义：然而；还；尚）",
                  "level": "red"
                },
                {
                  "word": "analog",
                  "ipa": "ˈænəlɒɡ",
                  "meaning": "adj.模拟的 n.模拟；类似物",
                  "level": "green"
                },
                {
                  "word": "record",
                  "ipa": "ˈrekɔːd",
                  "meaning": "adj.创记录的,空前的",
                  "level": "red"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": false
            },
            {
              "sIndex": 4,
              "id": "P2-S4",
              "text": "Libraries and archives have traditionally shouldered the responsibility of organization, preservation, and access to information. One of S.R. Ranganathan's foundational Laws of Library Science is \"Save the time of the reader.\" Thus, librarians digitize the tangible so that researchers the world over can quickly search and access their holdings. The result is an embarrassment of historical riches, which brings its own needle-and-haystack problems.",
              "translation": "图书馆与档案馆传统上一直默默肩负着信息组织整理、长期保存以及提供查阅的崇高神圣职责。图书情报学奠基人阮冈纳赞的基本法则之一便是“节省读者的时间”。因此，图书馆员们竭尽全力将有形实体文献全面数字化，使得全球各地的科研学者能够闪电般快速检索和调阅馆藏。其最终结果是造就了一座令人眼花缭乱的浩瀚历史文献宝库，但也随之带来了在浩如烟海中“大海捞针”的新难题。",
              "vocab": [
                {
                  "word": "responsibility",
                  "ipa": "riˌspɑnsəˈbɪləti",
                  "meaning": "n.责任,责任心;职责,任务",
                  "level": "red"
                },
                {
                  "word": "preservation",
                  "ipa": "ˌprezərˈveɪʃ(ə)n",
                  "meaning": "n.保存；保护；维持",
                  "level": "green"
                },
                {
                  "word": "access",
                  "ipa": "/ˈækses/",
                  "meaning": "获取、使用科研成果的机会或权利（常用释义：进入；接触；访问、获取）",
                  "level": "red"
                },
                {
                  "word": "information",
                  "ipa": "ɪnfəˈmeɪʃ(ə)n",
                  "meaning": "n. 信息",
                  "level": "green"
                },
                {
                  "word": "One",
                  "ipa": "/wʌn/",
                  "meaning": "其中一个（问题）（常用释义：一；一个人/事物）",
                  "level": "red"
                },
                {
                  "word": "Library",
                  "ipa": "ˈlaɪbrərɪ;(US)ˈlaɪbrerɪ",
                  "meaning": "n. 图书",
                  "level": "green"
                },
                {
                  "word": "Save",
                  "ipa": "prɛs",
                  "meaning": "v. 救助，搭",
                  "level": "green"
                },
                {
                  "word": "Thus",
                  "ipa": "/ðʌs/",
                  "meaning": "因此、从而（常用释义：以这种方式）",
                  "level": "red"
                },
                {
                  "word": "digitize",
                  "ipa": "ˈdɪdʒɪtaɪz",
                  "meaning": "v.[计] 数字化",
                  "level": "green"
                },
                {
                  "word": "tangible",
                  "ipa": "ˈtændʒəb(ə)l",
                  "meaning": "adj.明确的，真实的；可触摸的，可感知的",
                  "level": "green"
                },
                {
                  "word": "search",
                  "ipa": "sɜːtʃ",
                  "meaning": "n./v. 搜寻，搜查",
                  "level": "green"
                },
                {
                  "word": "embarrassment",
                  "ipa": "ɪmˈbærəsmənt",
                  "meaning": "n.尴尬，难堪；使人难堪的人/事；困境",
                  "level": "green"
                },
                {
                  "word": "needle-and-haystack",
                  "ipa": "",
                  "meaning": "adj.大海捞针的",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 3,
          "mainIdea": "图书馆员的无私奉献让手机随时可查，却导致用户产生图书馆不再重要的错觉，从而贬低低估了图书馆的不可替代价值",
          "logicRole": "指出数字化便捷让用户误以为图书馆不重要，导致图书馆价值被低估 (Undervaluation of Libraries)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P3-S1",
              "text": "Librarians' selfless devotion can act against us when users point to universality of access by holding up a cellphone and saying, \"it's all in here\" as evidence that libraries are less vital for researchers today.",
              "translation": "然而，图书馆员们的无私奉献在客观上却反噬了我们自身——当终端用户高举着智能手机、指着随时随地可触达的海量信息炫耀说“所有资料都在手机里了”并以此作为证据宣称“当今图书馆对科研人员已不再那么至关重要”时，这种极度盲目短视的言论导致图书馆的核心价值遭到了严重低估与贬损（undervaluation of libraries / libraries are less vital）。",
              "vocab": [
                {
                  "word": "selfless",
                  "ipa": "ˈselfləs",
                  "meaning": "adj.无私的，忘我的",
                  "level": "green"
                },
                {
                  "word": "devotion",
                  "ipa": "dɪˈvəʊʃn",
                  "meaning": "n.挚爱，忠诚；奉献；（宗教的）虔诚；祈祷，礼拜",
                  "level": "green"
                },
                {
                  "word": "act",
                  "ipa": "ækt",
                  "meaning": "v.表演;举动;起作用 n.行为,法令;一幕",
                  "level": "red"
                },
                {
                  "word": "universality",
                  "ipa": "ˌjuːnɪvɜːˈsæləti",
                  "meaning": "n.普遍性；广泛性；一般性；多方面性",
                  "level": "green"
                },
                {
                  "word": "access",
                  "ipa": "/ˈækses/",
                  "meaning": "获取、使用科研成果的机会或权利（常用释义：进入；接触；访问、获取）",
                  "level": "red"
                },
                {
                  "word": "evidence",
                  "ipa": "ˈevɪdəns",
                  "meaning": "n.证据,证明",
                  "level": "red"
                },
                {
                  "word": "vital",
                  "ipa": "ˈvaɪt(ə)l",
                  "meaning": "adj.至关重要的；充满活力的；生命的 n.身体的重要器官",
                  "level": "green"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            },
            {
              "sIndex": 2,
              "id": "P3-S2",
              "text": "Yet how was that universality of access made possible and, perhaps more importantly, how is it maintained? Who curates what is preserved? When it comes to born-digital information, the terrifying answer can be: if not librarians and archivists, then no one. Digital information requires a great deal more care than analog.",
              "translation": "然而，那种唾手可得的信息普适获取究竟是如何成为可能的？更重要的是，它又是靠谁在日复一日地严密维护？究竟是谁在精心筛选守护那些被留存的历史？当面对与生俱来的“原生数字信息”时，那令人不寒而栗的残酷答案只能是：如果没有图书馆员和档案管理学家的默默坚守，世上将再无任何人去拯救它们！数字信息实际上远比传统实体模拟信息需要多得多的悉心呵护与照料。",
              "vocab": [
                {
                  "word": "Yet",
                  "ipa": "/jet/",
                  "meaning": "然而；但是（常用释义：然而；还；尚）",
                  "level": "red"
                },
                {
                  "word": "universality",
                  "ipa": "ˌjuːnɪvɜːˈsæləti",
                  "meaning": "n.普遍性；广泛性；一般性；多方面性",
                  "level": "green"
                },
                {
                  "word": "access",
                  "ipa": "/ˈækses/",
                  "meaning": "获取、使用科研成果的机会或权利（常用释义：进入；接触；访问、获取）",
                  "level": "red"
                },
                {
                  "word": "information",
                  "ipa": "ɪnfəˈmeɪʃ(ə)n",
                  "meaning": "n. 信息",
                  "level": "green"
                },
                {
                  "word": "answer",
                  "ipa": "",
                  "meaning": "v.答复；n. 回答",
                  "level": "green"
                },
                {
                  "word": "one",
                  "ipa": "/wʌn/",
                  "meaning": "其中一个（问题）（常用释义：一；一个人/事物）",
                  "level": "red"
                },
                {
                  "word": "Digital",
                  "ipa": "ˈdɪdʒɪt(ə)l",
                  "meaning": "adj.数字的；数码的 n.数字电视",
                  "level": "green"
                },
                {
                  "word": "analog",
                  "ipa": "ˈænəlɒɡ",
                  "meaning": "adj.模拟的 n.模拟；类似物",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": false
            }
          ]
        },
        {
          "pIndex": 4,
          "mainIdea": "技术演进导致旧格式淘汰沦为无用废铁，ZIP 软盘示例生动展现了在淘汰过时格式中彻底丧失信息的巨大风险与断代噩梦",
          "logicRole": "以 ZIP 软盘为例证明过时格式导致信息丢失与历史记录中断的噩梦 (Loss in Obsolete Formats)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P4-S1",
              "text": "Even when a digital object is preserved, it may only be the carrier that's saved, not the information itself. As technology advances and a format becomes obsolete, the object is useless.",
              "translation": "即使某个数字物件侥幸被留存下来，最终被保存的往往也仅仅只是物理存储载体本身，而非内部承载的数据信息本身。随着科学技术的飞速迭代演进，一旦某种特定格式沦为过时淘汰（format becomes obsolete），该物件便彻底沦为了毫无用处的废品。",
              "vocab": [
                {
                  "word": "digital",
                  "ipa": "ˈdɪdʒɪt(ə)l",
                  "meaning": "adj.数字的；数码的 n.数字电视",
                  "level": "green"
                },
                {
                  "word": "object",
                  "ipa": "əbˈdʒekt",
                  "meaning": "n.物体，实物；目的，目标 v.反对",
                  "level": "red"
                },
                {
                  "word": "carrier",
                  "ipa": "/ˈkæriə(r)/",
                  "meaning": "承载者；传播者（常用释义：运载者；载体；携带者）",
                  "level": "red"
                },
                {
                  "word": "information",
                  "ipa": "ɪnfəˈmeɪʃ(ə)n",
                  "meaning": "n. 信息",
                  "level": "green"
                },
                {
                  "word": "format",
                  "ipa": "ˈfɔːmæt",
                  "meaning": "n.总体安排，计划，设计；开本，版式；格式v.为…编排格式；",
                  "level": "green"
                },
                {
                  "word": "obsolete",
                  "ipa": "ˈɒbsəliːt",
                  "meaning": "adj.淘汰的，废弃的；退化的",
                  "level": "green"
                },
                {
                  "word": "useless",
                  "ipa": "ˈjuːslɪs",
                  "meaning": "adj. 无用的",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": true
            },
            {
              "sIndex": 2,
              "id": "P4-S2",
              "text": "Have you ever stared helplessly at a ZIP disk, thinking: how do I get the files off this?",
              "translation": "你是否曾无比无助绝望地凝视着一张古早的 ZIP 软盘（ZIP disk），心中抓狂暗想：我究竟该如何把里面的文件导出来（possibility of losing information in obsolete formats）？",
              "vocab": [],
              "isTopicSentence": false,
              "isKeyEvidence": true
            },
            {
              "sIndex": 3,
              "id": "P4-S3",
              "text": "Without constant migration of digital assets, a nightmare about the foreseeable future is what keeps historians up at night: a historical record that abruptly stops when digital replaces analog.",
              "translation": "如果缺乏对数字资产持续不断的技术迁移迭代，在可预见的未来里，一场令所有历史学者彻夜难眠的终极噩梦就将不可避免地降临：当数字形态全面取代模拟实体的那一刻起，人类文明的全部历史记录竟然戛然而止、彻底中断！",
              "vocab": [
                {
                  "word": "constant",
                  "ipa": "ˈkɒnstənt",
                  "meaning": "adj.持续不断的，经常发生的；恒定的，不变的；忠诚的，忠",
                  "level": "green"
                },
                {
                  "word": "migration",
                  "ipa": "maɪˈɡreɪʃn",
                  "meaning": "n.移民，迁徙；（计算机）改变，迁移，转移",
                  "level": "green"
                },
                {
                  "word": "digital",
                  "ipa": "ˈdɪdʒɪt(ə)l",
                  "meaning": "adj.数字的；数码的 n.数字电视",
                  "level": "green"
                },
                {
                  "word": "nightmare",
                  "ipa": "ˈnaɪtmer",
                  "meaning": "n.噩梦；可怕的经历，很难对付的人/情形",
                  "level": "green"
                },
                {
                  "word": "about",
                  "ipa": "əˈbaʊt",
                  "meaning": "prep.从事于,忙于",
                  "level": "red"
                },
                {
                  "word": "foreseeable",
                  "ipa": "/fɔːˈsiːəbl/",
                  "meaning": "可预见的（常用释义：能预测的）",
                  "level": "red"
                },
                {
                  "word": "record",
                  "ipa": "ˈrekɔːd",
                  "meaning": "adj.创记录的,空前的",
                  "level": "red"
                },
                {
                  "word": "analog",
                  "ipa": "ˈænəlɒɡ",
                  "meaning": "adj.模拟的 n.模拟；类似物",
                  "level": "green"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        },
        {
          "pIndex": 5,
          "mainIdea": "保存数字历史记录是一项艰巨挑战，但图书馆员的坚守确保了数字人类记忆不会彻底湮灭",
          "logicRole": "总结全篇：保存数字历史记录充满巨大挑战，图书馆员发挥着中流砥柱作用 (Keeping Digital Records is a Challenge)",
          "sentences": [
            {
              "sIndex": 1,
              "id": "P5-S1",
              "text": "As a librarian whose day job revolves around special collections and digital assets, I share the night terrors of historians, and I'd be lying if I said a comprehensive preservation solution currently exists.",
              "translation": "作为一名日常工作深度围绕特藏文献与数字资产展开的职业图书馆员，我完全感同身受历史学者们内心的深夜梦魇；如果我宣称当前世上已存在某种一劳永逸的全面数字保存解决方案，那绝对是在撒谎欺骗。",
              "vocab": [
                {
                  "word": "librarian",
                  "ipa": "laɪˈbreərɪən",
                  "meaning": "n. 图书管理员；",
                  "level": "green"
                },
                {
                  "word": "around",
                  "ipa": "əˈraʊnd",
                  "meaning": "adv.存在着",
                  "level": "red"
                },
                {
                  "word": "special",
                  "ipa": "ˈspeʃ(ə)l",
                  "meaning": "adj. 特别的，专门的",
                  "level": "green"
                },
                {
                  "word": "digital",
                  "ipa": "ˈdɪdʒɪt(ə)l",
                  "meaning": "adj.数字的；数码的 n.数字电视",
                  "level": "green"
                },
                {
                  "word": "share",
                  "ipa": "ʃeər",
                  "meaning": "v. 分享，分",
                  "level": "green"
                },
                {
                  "word": "comprehensive",
                  "ipa": "ˌkɒmprɪˈhensɪv",
                  "meaning": "adj.综合性的；有理解力的 n.综合中学",
                  "level": "green"
                },
                {
                  "word": "preservation",
                  "ipa": "ˌprezərˈveɪʃ(ə)n",
                  "meaning": "n.保存；保护；维持",
                  "level": "green"
                },
                {
                  "word": "solution",
                  "ipa": "səˈluːʃ(ə)n",
                  "meaning": "n.解决方案；答案；溶液；溶解过程",
                  "level": "green"
                },
                {
                  "word": "currently",
                  "ipa": "ˈkɜːrəntli",
                  "meaning": "adv.目前；现在",
                  "level": "green"
                }
              ],
              "isTopicSentence": true,
              "isKeyEvidence": false
            },
            {
              "sIndex": 2,
              "id": "P5-S2",
              "text": "Yet researchers can take some comfort in the fact that there are a multitude of librarians devoted to discovering, organizing, and preserving digital information for researchers current and future. Librarians are uniquely positioned to understand how end users seek and use information. Thus we play an integral role in identifying, preserving, and providing accessibility to digital artifacts so that, while future researchers may find the digital realm a challenging place to ply their trade, they won't find it an impossible one.",
              "translation": "然而，广大科研学者尚可感到些许欣慰的是：世上仍有数以万计的图书馆员在全身心致力于为当今与未来的学者发掘、整理并守护保存宝贵的数字信息。图书馆员处于极其独特的枢纽地位，最懂得终端用户如何检索与使用信息。因此，我们在识别、保存并提供数字器物可获取性方面发挥着不可或缺的核心作用——从而确保，未来的科研人员在数字学术领域施展身手时，即便会发现这处领域充满重重艰巨挑战（Keeping digital historical records is a challenge），但也绝不至于绝望地发现这是一件彻底不可能完成的天方夜谭！",
              "vocab": [
                {
                  "word": "Yet",
                  "ipa": "/jet/",
                  "meaning": "然而；但是（常用释义：然而；还；尚）",
                  "level": "red"
                },
                {
                  "word": "take",
                  "ipa": "teɪk",
                  "meaning": "v. 携带，拿",
                  "level": "green"
                },
                {
                  "word": "multitude",
                  "ipa": "ˈmʌltɪtjuːd",
                  "meaning": "n.众多，大量；民众，群众，大批百姓；一大群人",
                  "level": "green"
                },
                {
                  "word": "devoted",
                  "ipa": "dɪˈvoʊtəd",
                  "meaning": "adj.献身的;忠诚的",
                  "level": "red"
                },
                {
                  "word": "digital",
                  "ipa": "ˈdɪdʒɪt(ə)l",
                  "meaning": "adj.数字的；数码的 n.数字电视",
                  "level": "green"
                },
                {
                  "word": "information",
                  "ipa": "ɪnfəˈmeɪʃ(ə)n",
                  "meaning": "n. 信息",
                  "level": "green"
                },
                {
                  "word": "for",
                  "ipa": "",
                  "meaning": "v.寻找;探索;渴望;盼望",
                  "level": "red"
                },
                {
                  "word": "understand",
                  "ipa": "",
                  "meaning": "v.理解，懂得；了解，明白；谅解，体谅；得知，听说；默认，默许；领会，推",
                  "level": "red"
                },
                {
                  "word": "end",
                  "ipa": "/end/",
                  "meaning": "目的；目标（常用释义：末端；结束；结局）",
                  "level": "red"
                },
                {
                  "word": "use",
                  "ipa": "",
                  "meaning": "n./v. 使用，用途",
                  "level": "green"
                },
                {
                  "word": "Thus",
                  "ipa": "/ðʌs/",
                  "meaning": "因此、从而（常用释义：以这种方式）",
                  "level": "red"
                },
                {
                  "word": "integral",
                  "ipa": "ˈɪntɪɡrəl",
                  "meaning": "adj.必需的，不可或缺的；完整的；作为组成部分的",
                  "level": "green"
                },
                {
                  "word": "accessibility",
                  "ipa": "əkˌsesəˈbɪləti",
                  "meaning": "n.易于进入/接近性；易使用性，可及性；易懂性",
                  "level": "green"
                },
                {
                  "word": "find",
                  "ipa": "faɪnd",
                  "meaning": "v.找到;发现;发觉",
                  "level": "red"
                },
                {
                  "word": "realm",
                  "ipa": "relm",
                  "meaning": "n.领域，范围；王国；（动物学）界",
                  "level": "green"
                },
                {
                  "word": "challenging",
                  "ipa": "ˈtʃælɪndʒɪŋ",
                  "meaning": "adj.具有挑战",
                  "level": "green"
                },
                {
                  "word": "place",
                  "ipa": "",
                  "meaning": "v.放置",
                  "level": "green"
                },
                {
                  "word": "ply",
                  "ipa": "plaɪ",
                  "meaning": "v.使用；定期地来往n.厚度；板层；褶",
                  "level": "green"
                },
                {
                  "word": "trade",
                  "ipa": "/treɪd/",
                  "meaning": "交换；用……换……（常用释义：贸易；交易；交换）",
                  "level": "red"
                },
                {
                  "word": "impossible",
                  "ipa": "",
                  "meaning": "adj.不可能的，办不到的；极其艰难的；难以对付的",
                  "level": "red"
                },
                {
                  "word": "one",
                  "ipa": "/wʌn/",
                  "meaning": "其中一个（问题）（常用释义：一；一个人/事物）",
                  "level": "red"
                }
              ],
              "isTopicSentence": false,
              "isKeyEvidence": true
            }
          ]
        }
      ],
      "questions": [
        {
          "qIndex": 36,
          "type": "修辞目的题",
          "tangchiModel": "唐迟修辞题模型：抓住第一段开篇提及古董店里老照片、旧书信等过往实体文物的由头（ephemera of history / still here because they are tangible），锁定作者提及这些过往实体文物是为了借实体长存引出数字时代文献资料长期保存的重大议题（bring up the issue of preservation）",
          "stem": "The author mentions the artifacts from the past to",
          "stemKeywords": [
            "mentions the artifacts from the past to"
          ],
          "targetSentences": [
            "P1-S1",
            "P1-S2",
            "P2-S1"
          ],
          "officialAnswer": "C",
          "options": [
            {
              "key": "A",
              "text": "introduce the coming of antiques",
              "isCorrect": false,
              "distractorType": "字面望文生义",
              "analysis": "【干扰特征: 字面局限】古董店只是引入话题的生活场景，非介绍古董行业的来临。",
              "refSentences": [
                "P1-S1"
              ]
            },
            {
              "key": "B",
              "text": "contrast them with everyday items",
              "isCorrect": false,
              "distractorType": "偷换对比对象",
              "analysis": "【干扰特征: 概念偏差】对比的是有形实体文物（tangible）与无形数字信息（intangible digital formats），非日常用品。",
              "refSentences": [
                "P1-S2",
                "P2-S1"
              ]
            },
            {
              "key": "C",
              "text": "bring up the issue of preservation",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 修辞功能定位】P1-S1 和 P1-S2 描写古董店里的照片信件历经数百年长存，并在 P2-S1 紧接着抛出核心论题：'Have you pondered the life cycle of intangible formats, digital information, given that those who produce these artifacts seldom make provision for their long-term preservation?'（你是否曾深思过数字信息的生命周期与长期保存问题？）。提及过往实体文物的长存正是为了极其自然地引出全文关于历史文献资料长期保存这一重大现实议题（bring up the issue of preservation）。C选项精准契合修辞目的。",
              "refSentences": [
                "P1-S1",
                "P1-S2",
                "P2-S1"
              ]
            },
            {
              "key": "D",
              "text": "comment on their historical value",
              "isCorrect": false,
              "distractorType": "以偏概全",
              "analysis": "【干扰特征: 次要表面】历史价值只是文物的属性，作者意在借其物理形态引出对数字化保存困境的深层思考。",
              "refSentences": [
                "P1-S2",
                "P2-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "tangible artifacts survive -> pondered life cycle of digital info long-term preservation = bring up the issue of preservation。",
            "methodSummary": "首段案例功能题：抓准实体长存引出的核心设问（long-term preservation），锁定 bring up the issue of preservation。"
          }
        },
        {
          "qIndex": 37,
          "type": "细节对比题",
          "tangchiModel": "唐迟细节题模型：抓住第二段关于有形实体文物与数字对象的对比（The thing itself is the medium that delivers the information / no intentional preservation needed），锁定有形实体文物本身就是物理载体，能以更为直接的方式传达历史信息（convey information in a more direct way）",
          "stem": "Compared with digital objects, tangible artifacts",
          "stemKeywords": [
            "Compared with digital objects",
            "tangible artifacts"
          ],
          "targetSentences": [
            "P2-S2",
            "P2-S3"
          ],
          "officialAnswer": "B",
          "options": [
            {
              "key": "A",
              "text": "are less subject to their creators' neglect",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实相反】实体文物同样经常遭到原作者的漠视疏忽（despite original creators' neglect），但仍能侥幸存活。",
              "refSentences": [
                "P2-S2"
              ]
            },
            {
              "key": "B",
              "text": "convey information in a more direct way",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P2-S2 明确阐述有形实体的独特属性：'For millennia, we've known what we've known due to artifacts that have survived... The thing itself is the medium that delivers the information'（实体器物本身就是承载并传递信息的物理介质，无需依赖解码设备直接肉眼可读）。The thing itself is the medium that delivers the information 与 convey information in a more direct way（以一种更为直接的方式传递信息）完全同义对应。B选项完全精准吻合。",
              "refSentences": [
                "P2-S2"
              ]
            },
            {
              "key": "C",
              "text": "require more intentional preservation",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实完全相反】P2-S3 和 P3-S2 明确指出实体无需刻意保存（no attempts at intentional preservation），反而是数字信息需要多得多的悉心维护照料（Digital info requires a great deal more care）。",
              "refSentences": [
                "P2-S3",
                "P3-S2"
              ]
            },
            {
              "key": "D",
              "text": "are less likely to suffer serious damage",
              "isCorrect": false,
              "distractorType": "无中生有物理损坏",
              "analysis": "【干扰特征: 偷换维度】实体也会风化损坏，文章强调的是实体信息与载体合一的直接性，非物理免损性。",
              "refSentences": [
                "P2-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "The thing itself is the medium that delivers the information = convey information in a more direct way。",
            "methodSummary": "原句属性同义改写：thing itself is the medium 转换为 convey information in a more direct way。"
          }
        },
        {
          "qIndex": 38,
          "type": "细节推断题",
          "tangchiModel": "唐迟细节题模型：抓住第三段首句关于图书馆员无私奉献带来的负面反噬效应（Librarians' selfless devotion can act against us when users hold up cellphone saying libraries are less vital），锁定图书馆员的辛勤工作反而导致了图书馆自身的核心价值遭到严重低估与贬低（undervaluation of libraries）",
          "stem": "According to Paragraph 3, librarians' work may result in",
          "stemKeywords": [
            "Paragraph 3",
            "librarians' work may result in"
          ],
          "targetSentences": [
            "P3-S1"
          ],
          "officialAnswer": "B",
          "options": [
            {
              "key": "A",
              "text": "oversupply of materials",
              "isCorrect": false,
              "distractorType": "跨段混淆",
              "analysis": "【干扰特征: 跨段错位】资料浩瀚（embarrassment of riches）是第2段末句描述的现象，第3段专讲这一奉献导致图书馆自身地位被用户轻视。",
              "refSentences": [
                "P2-S4",
                "P3-S1"
              ]
            },
            {
              "key": "B",
              "text": "undervaluation of libraries",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 同义替换】P3-S1 明确交代奉献带来的荒谬反噬后果：'Librarians' selfless devotion can act against us when users point to universality of access... saying \"it's all in here\" as evidence that libraries are less vital for researchers today'（馆员的奉献反噬了我们，当用户举起手机说都在这里面了并以此为证据认为图书馆如今已不再那么至关重要）。libraries are less vital 与 undervaluation of libraries（对图书馆价值的严重低估/贬低）完全同义对应。B选项精准吻合。",
              "refSentences": [
                "P3-S1"
              ]
            },
            {
              "key": "C",
              "text": "researchers' underperformance",
              "isCorrect": false,
              "distractorType": "无中生有",
              "analysis": "【干扰特征: 概念偏差】未提及科研人员业绩表现不佳。",
              "refSentences": [
                "P3-S1"
              ]
            },
            {
              "key": "D",
              "text": "users' overreliance on technology",
              "isCorrect": false,
              "distractorType": "次要表面现象",
              "analysis": "【干扰特征: 次要表象】举起手机是用户轻视图书馆的表象举动，段落核心强调的是对图书馆重要性的错误贬低与低估。",
              "refSentences": [
                "P3-S1"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "users say libraries are less vital for researchers today = undervaluation of libraries（对图书馆价值的低估）。",
            "methodSummary": "反噬效应提取：libraries are less vital 转换为 undervaluation of libraries。"
          }
        },
        {
          "qIndex": 39,
          "type": "例证题",
          "tangchiModel": "唐迟例证题模型：第四段引用面对过时的 ZIP 软盘无法读取文件的生动案例（stared helplessly at a ZIP disk: how do I get files off this? As a format becomes obsolete, the object is useless），是为了证明前句论点——在过时淘汰格式中丧失内部数据信息的巨大风险（the possibility of losing information in obsolete formats）",
          "stem": "The \"ZIP disk\" is cited as an example to show",
          "stemKeywords": [
            "\"ZIP disk\" is cited as an example to show"
          ],
          "targetSentences": [
            "P4-S1",
            "P4-S2",
            "P4-S3"
          ],
          "officialAnswer": "C",
          "options": [
            {
              "key": "A",
              "text": "the difficulty of retrieving files through unusual means",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 概念偏差】强调的是格式淘汰导致硬件无法读取从而丢失信息，非探讨非常规恢复手段的难度。",
              "refSentences": [
                "P4-S2"
              ]
            },
            {
              "key": "B",
              "text": "the infeasibility of constantly migrating digital assets",
              "isCorrect": false,
              "distractorType": "正反颠倒",
              "analysis": "【干扰特征: 事实相反】持续迁移数据资产是防止信息丢失的唯一必要手段（P4-S3），绝非不可行（infeasibility）。",
              "refSentences": [
                "P4-S3"
              ]
            },
            {
              "key": "C",
              "text": "the possibility of losing information in obsolete formats",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 例证论点对应】P4-S1 明确提出论点'As technology advances and a format becomes obsolete, the object is useless'（随着技术演进和某种格式沦为过时，该物品便彻底无用）；P4-S2 紧接着引述 ZIP 软盘作为直接例证：面对过时的 ZIP 软盘无助绝望，根本无法将文件导出。引用 ZIP 软盘正是为了生动论证：一旦存储格式过时淘汰，内部承载的历史信息就极可能彻底灭失丢失的严重风险（the possibility of losing information in obsolete formats）。C选项完全精准对应论点句。",
              "refSentences": [
                "P4-S1",
                "P4-S2"
              ]
            },
            {
              "key": "D",
              "text": "the inconvenience of storing information on analog devices",
              "isCorrect": false,
              "distractorType": "偷换属性为模拟设备",
              "analysis": "【干扰特征: 属性颠倒】ZIP 软盘是不折不扣的‘早期数字存储介质’（digital），非传统实体模拟设备（analog devices）。",
              "refSentences": [
                "P4-S1",
                "P4-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "format becomes obsolete, object is useless (helpless at ZIP disk) = the possibility of losing information in obsolete formats。",
            "methodSummary": "例证题前句论点锁定法：抓准 format becomes obsolete 对应的 losing information in obsolete formats。"
          }
        },
        {
          "qIndex": 40,
          "type": "主旨总结题",
          "tangchiModel": "唐迟主旨题模型：通读全文，提炼全篇围绕数字时代历史文献资产由于格式过时淘汰与缺乏维护而面临中断断代危机、保存数字历史记录充满巨大艰巨挑战的核心主旨（Keeping digital historical records is a challenge）",
          "stem": "Which of the following statements best summarizes the text?",
          "stemKeywords": [
            "best summarizes the text"
          ],
          "targetSentences": [
            "P2-S1",
            "P3-S2",
            "P4-S1",
            "P4-S3",
            "P5-S1",
            "P5-S2"
          ],
          "officialAnswer": "D",
          "options": [
            {
              "key": "A",
              "text": "Hard work should be done to preserve artifacts.",
              "isCorrect": false,
              "distractorType": "偏离数字核心",
              "analysis": "【干扰特征: 遗漏核心限定词】文章核心探讨的是‘原生数字历史记录’（digital historical records）的保存危机，非泛论传统实体文物（artifacts）。",
              "refSentences": [
                "P2-S1",
                "P4-S3"
              ]
            },
            {
              "key": "B",
              "text": "Contributions of librarians should be recognized.",
              "isCorrect": false,
              "distractorType": "以偏概全局部主体",
              "analysis": "【干扰特征: 次要角色】图书馆员的奉献是应对挑战的守护力量，全文通篇探讨的客观核心危机是数字历史记录的保存困境与挑战。",
              "refSentences": [
                "P3-S1",
                "P5-S2"
              ]
            },
            {
              "key": "C",
              "text": "Accessing databases is essential to researchers.",
              "isCorrect": false,
              "distractorType": "偷换概念",
              "analysis": "【干扰特征: 过于片面】文章重点在文献的‘长期留存与防灭失保存’（preservation），非仅仅谈数据库调阅。",
              "refSentences": [
                "P2-S4",
                "P5-S2"
              ]
            },
            {
              "key": "D",
              "text": "Keeping digital historical records is a challenge.",
              "isCorrect": true,
              "distractorType": null,
              "analysis": "【正确项 · 全文主旨高度凝练】全文通篇以“数字时代历史记录的保存危机”为唯一核心研讨主线：第1-2段通过实体与数字对比提出数字长期保存的严峻问题；第3段指出原生数字信息极其脆弱；第4段通过格式淘汰与 ZIP 软盘指出历史记录断代的深夜噩梦；第5段末句深刻总结指出未来科研人员将发现数字保存是一个充满巨大挑战的领域（challenging place）。D选项 Keeping digital historical records is a challenge（保存数字历史记录是一项极其艰巨的挑战）全面、客观且周延地概括了全文的主旨核心，为最佳总结项。",
              "refSentences": [
                "P2-S1",
                "P3-S2",
                "P4-S1",
                "P4-S3",
                "P5-S1",
                "P5-S2"
              ]
            }
          ],
          "presetReflection": {
            "trapAnalysis": "digital replaces analog nightmare of historical record abruptly stops + challenging preservation = Keeping digital historical records is a challenge。",
            "methodSummary": "全文核心主旨概括：识别文章核心对象（digital historical records）与核心论述属性（a challenge）。"
          }
        }
      ]
    }
  ]
};

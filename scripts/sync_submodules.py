...
用于一键按需拉取、全量同步或查看题库 13 个资源子模块（12 本书籍切图与英语精读 PDF）的本地状态。
"""
...
BOOKS = [
    ...
    "强化36讲", "822教材", "基础30讲", "1000题", "李范习题", "李范全书", "880", "英语精读PDF"
]
        ...
        if os.path.exists(bp):
            ...
            for r, d, fs in os.walk(bp):
                if '.git' in d:
                    d.remove('.git')
                for f in fs:
                    if f not in ('.gitignore', 'README.md'):
                        asset_files.append(os.path.join(r, f))
            f_count = len(asset_files)
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] == 'status':
        ...
        print("  python scripts/sync_submodules.py pull             # 一键拉取/更新全部 13 个子模块")
        print("  python scripts/sync_submodules.py init 880 英语精读PDF   # 按需仅拉取指定子模块")
